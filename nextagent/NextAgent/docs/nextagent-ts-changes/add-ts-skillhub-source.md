...
状态：ready
类型：实施 change
主要 owner：`agent-capability`、gateway remote port
依赖：`add-ts-capability-core-governance`、`add-ts-skill-manifest-contract`、`add-ts-agent-package-assembly`

目标：
- 支持 SkillHub 通过 remote gateway 进行显式 refresh、agent-scoped list/search、metadata fetch、package download；下载、校验、安装到 managed skills 目录并启用后才进入统一 catalog。
