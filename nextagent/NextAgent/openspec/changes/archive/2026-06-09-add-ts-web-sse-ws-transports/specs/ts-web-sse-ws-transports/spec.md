...
- **AND** 本 change MUST NOT 定义 cancel、answer pending input 或其他 lifecycle-changing WebSocket command
...
- **AND** selected transport MUST NOT 改变 `StreamEnvelope`、`StreamEventType`、session-scoped sequence、terminal event、`RuntimeTimelinePort.stream(request)` 或 runtime lifecycle semantics
...
Web stream projection MUST 具备 trusted identity、owner-scoped request coordinates、合法 transport request、`RuntimeTimelineStreamRequest`、`RuntimeTimelinePort.stream(request)`、safe projection rules 和 error normalization。Client-supplied owner fields MUST NOT 覆盖 trusted identity。
...
- **THEN** Web channel MUST 将其作为 `RuntimeTimelineStreamRequest.lastSeenSequence` 传入 runtime timeline stream
...
- **THEN** Web channel MUST abort 或 cleanup 对应 `RuntimeTimelinePort.stream(request)` subscription
...
- **WHEN** Web channel 无法消费 `RuntimeTimelinePort.stream(request)` 返回的 canonical timeline stream
