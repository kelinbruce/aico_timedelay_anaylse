# task-trajectory Specification

## Purpose
TBD - created by archiving change add-ts-task-trajectory. Update Purpose after archive.
## Requirements
...
#### Scenario: Bounded LLM note projection supports extraction
- **WHEN** a visible committed user message contains a concise telecom domain relation that deterministic extraction rules do not classify, such as a safe alarm-code-to-class relationship
- **THEN** the trajectory builder MAY persist a bounded, redacted `REQUEST_FACT` summary prefixed with `llm-note:`
- **AND** the projection MUST keep durable message source refs
- **AND** the projection MUST NOT include unsafe credentials, paths, raw tool output, attachment content, raw model output, or the full original message text
- **AND** downstream deterministic rule extraction MUST NOT treat the `llm-note:` prefix as a rule-ready memory candidate
