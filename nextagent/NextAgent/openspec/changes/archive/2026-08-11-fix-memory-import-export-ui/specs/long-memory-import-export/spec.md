...
- **变更类型**：修改
- **目标内容**：每次文件选择建立新的导入批次，当前批次内按原文件序号保持幂等；容量反馈由 ACTIVE 与 ARCHIVED 的个人设定记忆总数计算；导出按当前个人 Tab 呈现。
- **依据 Requirements**：`上传后必须预览、删除并确认导入`、`确认导入必须调用批量新增接口`、`导入结果必须准确报告部分成功和中断进度`、`筛选导出必须安全读取当前个人记忆结果`

### 结果

- **变更类型**：修改
- **目标内容**：用户主动重选相同文件可形成新写入批次，未知结果原样重试不重复写入；提示只陈述可验证的处理数量、个人余量和当前 Tab 导出数量。
...
|---|---|---|---|---|
| 导入批次幂等范围 | 修改 | 字节相同文件跨选择复用稳定 SHA-256 键 | 每次文件选择生成新批次 ID；同一 pending 批次内按原文件序号稳定，未知结果原样重试复用 | `确认导入必须调用批量新增接口`、`导入结果必须准确报告部分成功和中断进度` |
| 个人导入容量反馈 | 修改 | ACTIVE/ARCHIVED 的 `CONFIGURED` 数量加待导入数量，并在客户端阻断超限 | ACTIVE 与 ARCHIVED 的 `CONFIGURED` 总数相加，排除智能沉淀等非个人设定记忆，显示上限 50、已有数和 `max(0, 50-existing)`；服务端最终裁决 | `上传后必须预览、删除并确认导入` |
| 个人记忆导出呈现 | 修改 | 两个个人 Tab 共用“导出筛选结果”，共享 Tab 显示禁用按钮 | 我的/归档分别命名并显示实际数量成功提示，共享 Tab 不呈现个人导出操作 | `筛选导出必须安全读取当前个人记忆结果` |
