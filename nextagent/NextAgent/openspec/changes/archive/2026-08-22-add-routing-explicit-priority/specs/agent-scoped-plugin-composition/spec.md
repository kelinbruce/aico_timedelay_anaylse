...
- **变更类型**：修改
- **目标内容**：`agentRoutingPolicy` adapter 的委托行为从"plugin 优先且替代 built-in"改为"显式路由指定优先，plugin 与 built-in 平等处理无显式指定的策略选择"。
- **依据 Requirements**：`Policy plugins use an explicit open policy inventory`

### 处理过程

- **变更类型**：修改
- **目标内容**：adapter 先执行显式路由解析（指令、约束、policy 规则）。产生决策时直接采用，不调用路由策略实现。未产生决策时按 plugin 优先、built-in 兜底的顺序委托。
- **依据 Requirements**：`Policy plugins use an explicit open policy inventory`
