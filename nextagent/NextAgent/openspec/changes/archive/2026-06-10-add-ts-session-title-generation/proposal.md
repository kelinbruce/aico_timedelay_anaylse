...
- 新增 `GenerateSessionTitleCommand` DTO（`agent-contracts/session`，含 `identityContext`、`agentId`、`sessionId`、`requestRunId`、`firstUserText`、`isFirstRequest`）
- `UserSessionPort` 新增 `generateTitle(command: GenerateSessionTitleCommand): Promise<void>` 方法

### 领域层（agent-session）
- 实现确定性标题提取纯函数 `extractTitle(text)`：三级规则管线（短/中/长）
- `UserSessionService` 实现 `generateTitle`：isFirstRequest 判断 → loadSession 查 titleSource → extractTitle → redaction → saveSession（fire-and-forget）
- Runtime 传入 `firstUserText` 和 `isFirstRequest`，agent-session 不再自己查消息和 run 数
- 与 Owner 11 协作：标题内容经 redaction policy 检查后方可持久化

### Runtime 集成
- Runtime terminal commit 成功后调用 `userSessionPort.onRequestTerminal(sessionId, requestRunId, agentId)`
- `agent-app` composition 确保 `UserSessionPort` 注入到 runtime
...
| `agent-contracts/session` | `UserSessionPort.onRequestTerminal` 方法 | Owner 5 |
| `agent-session` | 标题提取纯函数 + `UserSessionService.onRequestTerminal` 实现 | Owner 5 |
| `agent-runtime` | terminal commit 后调用 `userSessionPort.onRequestTerminal` | Owner 2（协作） |
| `agent-app` | composition 注入 `UserSessionPort` 到 runtime | Owner A（协作） |
| `agent-platform-gateway-local` | `saveSession` 自动保留 `titleSource` 字段（JSON 序列化，无需代码改动） | 无需改动 |
