...
在 collaborative 模式下，扩展面板 SHALL 永远位于左侧，填充 PIU 对话面板之外的剩余区域。扩展面板打开时，PIU 对话面板 MUST 强制变为 docked-right 状态，宽度 MUST 重置为 `DOCKED_DEFAULT_WIDTH` 或 `AICOConfig.modalSize.width`（如果配置了）。如果 PIU 面板当前为 floating 或 maximized 状态，MUST 先切换为 docked 再设置宽度。`expandPanelPosition` 配置在 collaborative 模式下 MUST 被忽略。关闭扩展面板后，PIU 面板宽度 MUST 保持默认值，不恢复用户之前拖拽的宽度。
...
- **GIVEN** collaborative 模式下扩展面板已打开，PIU 面板为默认宽度
- **WHEN** 扩展面板关闭
- **THEN** PIU 面板宽度 MUST 保持默认值
