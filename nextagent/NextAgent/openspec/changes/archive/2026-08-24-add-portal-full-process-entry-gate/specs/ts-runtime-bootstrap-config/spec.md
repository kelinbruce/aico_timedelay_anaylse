...
- **变更类型**：修改
- **目标内容**：runtime bootstrap response 的 `portalAbilityConfig` 始终返回 `cronTasksEnabled`、`longTermMemoryManagementEnabled`、`knowledgeImportEnabled` 和 `fullProcessEnabled`，不返回 AskUserQuestion 等待时间。
