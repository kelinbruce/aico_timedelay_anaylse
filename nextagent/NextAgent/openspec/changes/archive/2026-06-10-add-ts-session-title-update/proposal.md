...
- `add-ts-session-title-generation` 引入了 `titleSource?: "automatic" | "manual"` 字段和 `UserSessionPort.generateTitle`
...
- 新增 `UpdateSessionTitleCommand` DTO（`identityContext`、`agentId`、`sessionId`、`title`、`idempotencyKey`）
- `UserSessionPort` 新增 `updateTitle(command: UpdateSessionTitleCommand): Promise<UserSession>`

### 领域层（agent-session）
- `UserSessionService` 实现 `updateTitle`：长度校验（≤100 字符）→ redaction 检查 → owner+agent scope 校验 → saveSession（titleSource="manual"）→ audit
- title 为空时允许清空标题，titleSource 仍设为 "manual"
...
2. `UserSessionPort.generateTitle` 方法——本 change 的 `titleSource="manual"` 语义需要被 title-generation 的跳过逻辑正确识别
