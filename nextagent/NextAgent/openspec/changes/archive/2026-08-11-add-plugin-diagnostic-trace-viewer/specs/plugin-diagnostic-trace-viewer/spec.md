...
- **变更类型**：新增
- **目标内容**：系统允许 Agent 开发者和运维人员离线导入插件开发诊断 NDJSON，并按会话与请求复盘有序执行轨迹。
- **依据 Requirements**：`查看器按会话和请求区分执行轨迹`、`查看器以确定顺序呈现轨迹事件`、`单行错误只降级当前记录`、`查看过程保持本地只读边界`、`查看器作为本地运行包的插件伴随文件交付`

### 前置条件

- **变更类型**：新增
- **目标内容**：使用者能够访问查看器文件和一份本地 UTF-8 NDJSON 文件。
- **依据 Requirements**：`查看器按会话和请求区分执行轨迹`、`查看过程保持本地只读边界`

### 输入

- **变更类型**：新增
- **目标内容**：使用者选择的一份本地 UTF-8 NDJSON 文件。
- **依据 Requirements**：`查看器按会话和请求区分执行轨迹`

### 输出

- **变更类型**：新增
- **目标内容**：文件中的全部轨迹列表、选定轨迹的有序事件流程、阶段核心指标、原始记录详情和被忽略记录的问题摘要。
- **依据 Requirements**：`查看器按会话和请求区分执行轨迹`、`查看器以确定顺序呈现轨迹事件`、`查看器按事件阶段展示核心指标`、`单行错误只降级当前记录`

### 处理过程

- **变更类型**：新增
- **目标内容**：系统逐行校验输入，以 `(sessionId, requestId)` 精确分组合法记录，确定事件顺序，按 stage 映射核心指标并呈现选定轨迹；非法记录被隔离并报告。
- **依据 Requirements**：`查看器按会话和请求区分执行轨迹`、`查看器以确定顺序呈现轨迹事件`、`查看器按事件阶段展示核心指标`、`单行错误只降级当前记录`

### 结果

- **变更类型**：新增
- **目标内容**：使用者无需启动 NextAgent 或上传调测内容即可切换并复盘文件中的每条合法执行轨迹。
- **依据 Requirements**：`查看过程保持本地只读边界`、`查看器作为本地运行包的插件伴随文件交付`
...
- **规格项**：轨迹标识
  - **变更类型**：新增
  - **原规格值**：不适用（新增）
  - **目标规格值**：顶层 `(sessionId, requestId)` 精确有序组合
  - **依据 Requirements**：`查看器按会话和请求区分执行轨迹`
- **规格项**：事件排序
  - **变更类型**：新增
  - **原规格值**：不适用（新增）
  - **目标规格值**：有效 `recordedAt` 升序、同时间按行号升序；无效时间最后按行号升序
  - **依据 Requirements**：`查看器以确定顺序呈现轨迹事件`
- **规格项**：运行依赖
  - **变更类型**：新增
  - **原规格值**：不适用（新增）
  - **目标规格值**：单个本地 HTML；无 NextAgent 服务、网络和外部资源依赖
  - **依据 Requirements**：`查看过程保持本地只读边界`

### 覆盖特性

- **变更类型**：新增
- **目标内容**：`F-10.2 装配插件`
- **依据 Requirements**：`查看器作为本地运行包的插件伴随文件交付`

### 主规格

- **变更类型**：新增
- **目标内容**：`plugin-diagnostic-trace-viewer`
- **依据 Requirements**：`查看器按会话和请求区分执行轨迹`、`查看器以确定顺序呈现轨迹事件`、`查看器按事件阶段展示核心指标`、`单行错误只降级当前记录`、`查看过程保持本地只读边界`、`查看器作为本地运行包的插件伴随文件交付`
