...
- **变更类型**：修改
- **目标内容**：会话派生接收可信源会话、独立消息或请求锚点、规范化幂等键和fork attempt；不接收完整WorkingMemory前缀或预构造子会话事实，只在promotion stage接收prepare清单对应的受预算bytes。
- **依据 Requirements**：`会话派生 Runtime facade 保持可信窄入口`、`会话派生 gateway 公开准备与原子创建入口`、`会话派生跨 provider 边界使用有界协调材料`、`Fork From Durable Visible Assistant Message`、`Fork Idempotency`

### 输出

- **变更类型**：修改
- **目标内容**：派生成功时返回已提交 child session 和幂等重放状态；取消或失败时 reject 安全 `AgentError` 且不产生可见部分 child。
- **依据 Requirements**：`会话派生 gateway 公开准备与原子创建入口`、`会话派生失败使用唯一安全错误契约`、`Fork Idempotency`、`Fork Failure Is Atomic And Safe`

### 处理过程

- **变更类型**：修改
- **目标内容**：系统根据可信坐标在Working Memory provider内读取完整前缀并返回有界ref清单，NextAgent只解析和stage清单内容，随后系统完成安全重映射、模型可见上下文、过程快照、matching promotion与child原子提交；任一失败不产生可见child facts。
- **依据 Requirements**：`会话派生跨 provider 边界使用有界协调材料`、`会话派生来源元数据保持窄化`、`Child Session Inherits Prefix And Model-Visible Context`、`Fork Idempotency`、`Fork atomically materializes child-owned process history`、`Fork Failure Is Atomic And Safe`

### 结果

- **变更类型**：修改
- **目标内容**：LOCAL 与 REMOTE 对相同派生事实产生结构等价的 child session 或一致的安全失败，不因部署模式改变完整前缀语义。
- **依据 Requirements**：`LOCAL 与 REMOTE 会话派生保持契约一致`、`会话派生跨 provider 边界使用有界协调材料`

### 接口

- **变更类型**：修改
- **目标内容**：Runtime会话派生入口保持不变；gateway创建边界收敛为`prepareFork`、既有promotion staging与最终`forkSession`；创建后读取和维护继续使用保留的窄操作。
- **依据 Requirements**：`会话派生 Runtime facade 保持可信窄入口`、`会话派生 gateway 公开准备与原子创建入口`

### 规格

- **变更类型**：修改
- **规格项**：复制范围
- **原规格值**：锚点内消息前缀及其可用只读过程快照；不复制可操作运行状态
- **目标规格值**：从源会话开头到最终消息锚点的完整 canonical durable 消息前缀、由 copied child prefix 初始化的 model-visible active context 及可用只读过程快照；不复制可操作运行状态
- **依据 Requirements**：`会话派生跨 provider 边界使用有界协调材料`、`Child Session Inherits Prefix And Model-Visible Context`、`Fork atomically materializes child-owned process history`

- **变更类型**：新增
- **规格项**：派生请求材料
- **原规格值**：不适用（新增）
- **目标规格值**：prepare与fork只接受可信身份、源会话、独立消息或请求锚点、幂等键和fork attempt；不传完整历史，ref清单与promotion bytes分别受既有数量和bytes预算约束
- **依据 Requirements**：`会话派生 Runtime facade 保持可信窄入口`、`会话派生 gateway 公开准备与原子创建入口`、`会话派生跨 provider 边界使用有界协调材料`

- **变更类型**：新增
- **规格项**：跨部署结果
- **原规格值**：不适用（新增）
- **目标规格值**：LOCAL 与 REMOTE 对相同可信范围、源事实、锚点、幂等键和预算产生 alpha-equivalent child 结果，或 reject 相同 canonical error tuple，且不留下部分 child
- **依据 Requirements**：`LOCAL 与 REMOTE 会话派生保持契约一致`、`会话派生失败使用唯一安全错误契约`、`Fork Failure Is Atomic And Safe`

### 遗留规格

- **变更类型**：修改
- **目标内容**：`ts-core-contracts` 只继续承载本 change 未触及的 legacy fork contracts；被触及的创建、prefix、projection、active context 和 promotion 编排契约由 canonical `session-fork-from-message` 承载。
- **依据 Requirements**：`会话派生 gateway 公开准备与原子创建入口`、`会话派生失败使用唯一安全错误契约`、`会话派生来源元数据保持窄化`、`Child Session Inherits Prefix And Model-Visible Context`、`Fork Idempotency`、`Fork Failure Is Atomic And Safe`
