...
### D1：执行模型 — 统一的 `updateTitle` 方法

`UserSessionPort.updateTitle` 是 session domain 统一的标题写入方法，接收 `titleSource` 参数区分来源：

```ts
interface UpdateSessionTitleCommand {
  readonly identityContext: IdentityContext;
  readonly agentId: AgentId;
  readonly sessionId: SessionId;
  readonly title: string;
  readonly titleSource: SessionTitleSource;  // "automatic" | "manual"
  readonly idempotencyKey: IdempotencyKey;
}
```

- 用户手动修改：Channel endpoint 传 `titleSource="manual"`
- Runtime 自动生成：Runtime 传 `titleSource="automatic"`（见 `add-ts-session-title-generation`）

agent-session 内部逻辑根据 `titleSource` 分两条分支：
- `"automatic"`：先 loadSession 检查当前 titleSource——若已是 `"manual"` 或 title 非空则跳过
- `"manual"`：直接通过（用户显式操作，总是允许）

两条分支共享 redaction → saveSession → audit 路径。audit event 名称区分：`session.title.generated` vs `session.title.updated`。

### D2：覆盖保护 — titleSource = "manual"

用户通过 Channel 修改标题时传 `titleSource="manual"`。此后 `add-ts-session-title-generation` 的自动生成逻辑（`updateTitle` 中 `titleSource="automatic"` 分支）检测到已有 manual title 即跳过。
