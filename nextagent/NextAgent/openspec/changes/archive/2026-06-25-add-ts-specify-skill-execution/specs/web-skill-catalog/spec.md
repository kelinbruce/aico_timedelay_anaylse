...
API SHALL 支持可选的 case-insensitive 关键字模糊搜索。当 `keyword` 参数提供时，API MUST 过滤结果为 `displayName` 或 `capabilityId` 包含该关键字子串（忽略大小写）的 Skill。API MUST NOT 搜索 `description`、`inputSchema`、`outputSchema`、`metadata`、provider 配置或其他非可见字段。关键字搜索 MUST NOT 绕过 scope、availability 或 governance 检查。`keyword` 为空字符串或仅空白时 MUST 等同于不提供 `keyword`。
...
- **AND** 结果 MUST NOT 包含 `displayName` 和 `capabilityId` 均不包含 "alarm" 的 Skill
...
- **AND** 没有任何 Skill 的 `displayName` 或 `capabilityId` 包含 "zzzzz"
