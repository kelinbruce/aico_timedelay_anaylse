...
`MessageInput` SHALL 在输入普通文本（非 `/` 开头且 trim 后非空）时显示联想面板。联想面板 MUST NOT 在以下情况触发：
- 输入以 `/` 开头（斜杠命令面板优先）
- 输入为空或 trim 后为空

联想面板与斜杠命令面板互斥，MUST NOT 同时显示。

#### Scenario: 输入普通文本触发联想
- **WHEN** 用户输入 "告警"
- **THEN** 联想面板 MUST 显示
- **AND** 斜杠命令面板 MUST NOT 显示

#### Scenario: 输入斜杠命令不触发联想
- **WHEN** 用户输入 "/he"
- **THEN** 斜杠命令面板 MUST 显示
- **AND** 联想面板 MUST NOT 显示

#### Scenario: 输入为空不触发
- **WHEN** 输入框为空或仅空白
- **THEN** 联想面板 MUST NOT 显示
- **AND** 斜杠命令面板 MUST NOT 显示
