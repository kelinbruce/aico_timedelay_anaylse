<!--
本文件是 ts-architecture-test-gate change 的行为规格 delta。
Capability 按维度划分为 4 个 spec 文件。
-->
...
NextAgent TS 后端与配套 Web UI MUST 在交互维度满足本行为契约，验证 SSE 渲染、Pending Input 交互、断连重连、Session List、Composer 草稿、深色模式等行为。

#### Scenario: Web UI 提交消息→SSE stream 推送回复正确渲染（正路径）
- **WHEN** trusted identity + create session + 在 Web UI Composer 输入文本并提交
- **THEN** Composer 清空、turn-block-skeleton 可见、SSE stream 完成后 ai-bubble + chat-stream-status-strip 可见

#### Scenario: Web UI Pending Input AUTHORIZATION 交互组件渲染与响应（正路径）
- **WHEN** Submit 触发 pending input
- **THEN** respond-input-panel + respond-input-approval 可见，点击 approval 后 stream 完成

#### Scenario: Web UI Stream 断连重连提示与内容恢复（正路径）
- **WHEN** SSE 断连后重连
- **THEN** chat-stream-status-strip 可见（降级提示），重连后 stream 完成

#### Scenario: Web UI 会话列表展开/收缩 sessionStorage 持久化（正路径）
- **WHEN** 创建 5 个 session 后展开会话列表
- **THEN** ⚠️ 已知问题: sessionStorage.getItem('sessionListPreference') 返回 null（前端不使用此 key）

#### Scenario: Web UI Composer 草稿缓存与恢复（正路径）
- **WHEN** 在 Composer 输入文本后切换 session
- **THEN** ⚠️ 已知问题: sessionStorage.getItem('composerDraft_{sid}') 返回 null（前端不使用此 key）

#### Scenario: Web UI 深色模式 scrollbar 主题一致性（正路径）
- **WHEN** 切换深色模式后检查 scrollbar 风格
- **THEN** ⚠️ 已知问题: 前端无 theme-toggle-btn，深色模式切换入口未确定
