...
状态：ready
类型：发布门槛 change
主要 owner：`agent-app`、架构 owner
依赖：首版必需 change 完成

目标：
- 自动检查模块边界、依赖方向、跨层绕行和实现包泄漏，防止破坏架构设计。
- 维护唯一标准命令 `npm run lint:architecture`；`harden-ts-local-runtime-release` 直接使用其退出状态，命令可按 `NEXTAGENT_RELEASE_CHECK_DIR` 协议补充安全报告，但不得要求 release-only architecture 检查实现。
...
后续维护：
- 本 change 负责 architecture 检查内容、标准命令和报告；调用顺序与 verdict 聚合归 `harden-ts-local-runtime-release`。
