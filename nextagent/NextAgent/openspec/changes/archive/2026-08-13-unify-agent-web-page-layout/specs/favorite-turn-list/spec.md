...
收藏主内容 MUST 使用 `agent-web-page-layout` 定义的 Header，并 MUST 从 Header 左侧内容起始位置展示“收藏”标题，MUST NOT 在标题前展示页内返回操作。收藏 Header 的高度、水平内边距、标题字体、主题和窄视口行为 MUST 由 `agent-web-page-layout` 唯一定义，收藏页面 MUST NOT 再覆盖这些视觉规格。离开收藏内容、切换路由或关闭 PIU 左侧扩展内容容器 MUST 继续由当前宿主已有的导航或容器关闭入口决定，收藏页面 MUST NOT 新增平行导航 authority。
...
- **AND** Header MUST 从左侧内容起始位置展示“收藏”标题
...
- **变更类型**：修改
- **目标内容**：用户在 Local、Immersive 和 Collaborative/PIU 主内容区域查看收藏列表时，收藏页面使用不带页内返回操作的统一 Agent Web Header，并保留既有过滤、分组、分页、展开正文、取消收藏和宿主导航行为。
- **依据 Requirements**：`Local 与 Immersive 收藏内容视图`

### 处理过程

- **变更类型**：修改
- **目标内容**：系统在统一 Header 中仅展示收藏标题；离开收藏、切换路由或关闭 PIU 扩展容器继续由宿主已有入口处理，其余收藏处理过程保持不变。
- **依据 Requirements**：`Local 与 Immersive 收藏内容视图`

### 结果

- **变更类型**：修改
- **目标内容**：收藏页面的标题栏与其他首批 Agent Web 主内容页面保持同形且不展示页内返回操作；该调整不改变收藏页面的读取、分页、展开、写入和失败结果。
- **依据 Requirements**：`Local 与 Immersive 收藏内容视图`
