...
状态：active
类型：实施 change
主要 owner：`agent-context-engine`、`BlobStoreGateway`、gateway artifact port
依赖：`add-ts-context-budget-explainability`

目标：
- 支持附件、大的工具调用结果和模型摘要按需引用或加载，而不是全部内联。
...
- 大内容 bytes 统一通过 `BlobStoreGateway` 保存并返回 opaque `BlobRef`；附件、artifact、大 capability result 和其他大对象复用同一个 blob store 边界。
- `BlobRef` 不得进入模型上下文、用户可见 stream、SafeError、audit 明细或结构化日志；Context Engine 只能暴露安全 descriptor、受控内容片段、summary 或业务级 ContentRef。
- 用户输入附件通过 `attachmentIds` 和权威 `RequestAttachment` 查询进入上下文，不通过 artifact metadata 或请求体中的附件描述进入上下文。
