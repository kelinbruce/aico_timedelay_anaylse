...
系统 MUST 通过统一 capability discovery 和 catalog governance 边界发现 builtin Skills。本 change 中的 builtin Skills 必须归一为 `SKILL` capability，并通过 `CapabilityCatalog` 注册为受治理 descriptor。
...
Trusted bundled resource root 是本 change 的 builtin Skill archive boundary。系统必须把 root 视为只读 discovery source material，并保持物理路径、resource URI 和 package internal layout 为 provider-private implementation fact。
...
- **AND** 嵌套目录不会在本 change 中成为独立 builtin Skill candidate
...
Builtin Skill discovery MUST 由 `agent-capability` 内的 `BuiltinSkillDiscovery` adapter 负责。`BuiltinSkillDiscovery` 必须实现现有 `CapabilityDiscovery` contract，使用 `discoveryMode=EAGER`，并只能由 `DefaultCapabilityDiscoveryFactory` 在 trusted provider input 精确匹配 `providerId="builtin-skills"` 和 `providerKind=BUNDLED` 时创建。
...
#### Scenario: Factory 为可信 provider 创建 builtin Skill discovery

- **WHEN** capability subsystem 请求 `DefaultCapabilityDiscoveryFactory` 为 `providerId="builtin-skills"`、`providerKind=BUNDLED` 和 `discoveryMode=EAGER` 创建 discovery
- **THEN** factory 必须创建 `BuiltinSkillDiscovery`
...
- **THEN** `DefaultCapabilityDiscoveryFactory` 不得为该 provider 创建 builtin Skill discovery
...
- **AND** runtime relationship 必须由 capability governance 和 invocation execution 相关 change 定义
...
首个 TS 发布版本 MUST 把必需 builtin Tools 和 builtin telecom Skills 作为 framework-default Agent capabilities。Framework-default builtin capability 的默认启用 MUST 由 Capability Catalog 基于 trusted builtin provider descriptors 计算。Agent App MUST 负责校验并传递用户显式 binding facts。首版 framework-default builtin Tool 必须至少包含 `providerId=builtin-tools`、`capabilityType=TOOL` 的 `read` capability；首版 framework-default builtin Skill 必须包含 `providerId=builtin-skills`、`capabilityType=SKILL`、stable identity `telecom-domain-qa` 的 required telecom Skill。
...
Agent App assembly validation MUST 校验 binding shape、安全 id、受支持 capability type 和 provider id 有效性，并把用户显式 enabled / disabled binding facts 传递给 runtime-facing assembly。Runtime-facing `AgentCapabilityBinding.enabled` MUST 是可选字段；缺省值 MUST 等价于 `enabled=true`，`enabled=false` MUST 表达显式禁用。Provider id 有效性 MUST 来自 trusted startup resource provider registry。Capability existence、availability、conflict resolution、model visibility 和 invocation eligibility MUST 由 Capability Catalog 在 `listAvailable` / `resolve` 阶段决定。
...
#### Scenario: 每个 Agent 默认获得 framework-default builtin capability 可见性

- **WHEN** 首个 TS 发布版本的 Capability Catalog 为某个 Agent 计算 `listAvailable` 或 `resolve`
- **AND** trusted builtin Tool / Skill descriptors 已被 governance 接受
- **THEN** catalog 必须把 framework-default builtin Tool 和 builtin Skill 作为该 Agent 的默认候选
- **AND** builtin Tool candidate 必须使用 `capabilityType=TOOL`
- **AND** builtin Skill candidate 必须使用 `providerId=builtin-skills`、`capabilityType=SKILL` 和 stable identity `telecom-domain-qa`
...
#### Scenario: Agent App 传递 explicit binding facts

- **WHEN** Agent App 装配 Agent assembly
- **THEN** 它必须基于 Agent definition 和 trusted startup resource provider registry 完成 binding validation
...
#### Scenario: Binding 在装配阶段只做 shape、type、provider 有效性检查

- **WHEN** Agent definition 包含 explicit enabled 或 disabled binding
- **THEN** Agent App assembly validation 必须校验 binding shape、安全 `capabilityId`、受支持 `capabilityType` 和 `providerId` 格式
- **AND** `providerId` 必须匹配 trusted startup resource provider registry 中已注册 provider
...
- **WHEN** Agent definition capability binding 使用未出现在 trusted startup resource provider registry 中的 `providerId`
- **THEN** parsing 或 assembly compilation 必须安全拒绝
- **AND** 系统不得把该 provider id 解释为 lazy provider、builtin provider alias 或 descriptor metadata lookup

### Requirement: Builtin Skill Provider 只通过可信 Composition 注册

`builtin-skills` provider MUST 通过可信 framework composition 注册。`agent-capability` 必须定义 trusted provider identity，`DefaultCapabilityDiscoveryFactory` 必须为该 provider 创建 `BuiltinSkillDiscovery`，`createCapabilitySubsystem()` 必须把 builtin Skill eager discovery 加入 catalog startup discoveries，`agent-app` startup resource provider registry 必须注册 `builtin-skills` provider registration，用于 resource inventory、readiness 和 binding provider id validation。
...
#### Scenario: Startup composition 注册 builtin skills provider

- **WHEN** product app 创建 capability subsystem 和 startup resource provider registry
- **THEN** subsystem 必须包含 `builtin-skills` 的 trusted eager discovery
- **AND** startup resource provider registry 必须包含 `providerId="builtin-skills"` 和 `providerKind=BUNDLED`
- **AND** 两处注册都必须来自 trusted framework composition
...
Builtin Skill readiness 或 health diagnostics MUST 是 implementation-local startup evidence，由 `agent-capability` 和 trusted app startup/readiness composition 拥有。本 change 不新增 Web API、stream event、audit schema、metric schema 或 `agent-contracts` public readiness DTO。
...
### Requirement: 本 Change 覆盖 Discovery、Catalog 默认注入和 Explicit Disable，不定义 Skill Execution

本 change MUST 只定义 builtin Skill discovery、validation、catalog registration、catalog default builtin injection 和 explicit disable application。Skill inline execution、fork execution、progressive disclosure、nested invocation、audit、idempotency、sandbox behavior 和 remote SkillHub protocol behavior 由后续对应 change 定义。
...
- **AND** execution semantics 必须由 capability execution 和 governance 相关 change 定义

### Requirement: Alpha 包含可发现的 Framework-Default 电信 Builtin Skill Candidate

首个 TS 发布版本 MUST 包含 framework-default builtin telecom Skill candidate `telecom-domain-qa`，并具备稳定 manifest identity `name: telecom-domain-qa`。Release readiness 必须使用该 identity 判断 required candidate 是否存在、manifest-valid，并被 governance 解析为 available 或 degraded outcome。

`telecom-domain-qa` 表示电信领域问答与诊断意图澄清的默认能力 identity。本 requirement 只覆盖 discovery、descriptor exposure、catalog default injection 和 readiness，不定义该 Skill 的知识内容、prompt 内容、model behavior 或 invocation behavior。

#### Scenario: telecom-domain-qa builtin Skill candidate 可发现

- **WHEN** builtin Skill source discovery 在 trusted source enablement active 状态下运行
- **THEN** 系统必须把 `telecom-domain-qa` framework-default telecom Skill candidate 暴露给 capability governance
- **AND** candidate 必须使用标准 `SKILL.md` manifest contract
- **AND** manifest stable identity 必须是 `name: telecom-domain-qa`

#### Scenario: 缺失 required telecom builtin Skill 在 readiness 中可见

- **WHEN** required framework-default telecom builtin Skill candidate `telecom-domain-qa` missing、manifest-invalid 或 governance 后 unavailable
- **THEN** release readiness 或 health diagnostics 必须暴露安全 failure 或 degraded outcome
- **AND** 系统不得静默报告 builtin Skill source fully ready

### Requirement: Builtin Skill Diagnostics 安全可解释

Builtin Skill source diagnostics MUST 解释 source disabled、candidate ignored、missing manifest、invalid manifest、unavailable after governance 和 successful registration outcome，并保持输出安全。
...
- **WHEN** builtin Skill discovery 和 capability governance 成功注册 required telecom builtin Skill `telecom-domain-qa`
- **THEN** release readiness 或 health diagnostics 必须为 stable Skill identity `telecom-domain-qa` 暴露安全 successful-registration outcome
