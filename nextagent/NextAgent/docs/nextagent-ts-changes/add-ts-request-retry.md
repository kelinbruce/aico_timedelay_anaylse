...
状态：active
类型：实施 change
主要 owner：`agent-runtime`、`agent-session`
依赖：`ship-ts-minimal-agent-kernel`

目标：
- 支持对最近完成请求创建新的执行尝试，并保留旧结果可追溯。
...
- `RuntimeCommandPort.retryLatest(command)` 返回 `RequestAccepted`，因为 retry 会创建新的 run/attempt。
...
- retry 替换旧请求结果时，必须通过 `SessionMessageStoreGateway.hideMessage(HideMessageRequest)` 隐藏旧默认历史视图中的被替换 message；不得通过重新保存整条 message 或私有 update 改写 visibility。
