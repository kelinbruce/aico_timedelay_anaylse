...
17. FavoriteTurnsPanel per-turn unfavorite control SHALL be disabled.
18. FavoriteTurnsPanel batch-remove-favorites entry SHALL be disabled, and the batch removal confirmation path SHALL NOT be reachable without `AICOServiceOperation.Write`.
...
#### Scenario: 收藏列表页面写操作在缺少 Write 时禁用
- **GIVEN** 远程只读用户拥有 `AICOService.View` 但缺少 `AICOService.Write`
- **WHEN** 收藏列表页面渲染
- **THEN** 列表项的取消收藏控件 MUST 被禁用
- **AND** 页头批量取消收藏入口 MUST 被禁用
- **AND** 批量取消收藏确认路径 SHALL 不可达
- **AND** 禁用控件 MUST 展示权限提示

#### Scenario: 收藏列表页面写操作在有 Write 时可用
- **GIVEN** 远程用户拥有 `AICOService.View` 和 `AICOService.Write`
- **WHEN** 收藏列表页面渲染
- **THEN** 单个取消收藏和批量取消收藏入口 SHALL 不被权限门禁禁用
- **AND** 既有业务禁用条件（`batchMode`、`pendingRemovalKey` 等）MUST 继续优先于权限门禁

#### Scenario: 收藏列表页面只读操作不受权限影响
- **GIVEN** 远程只读用户拥有 `AICOService.View` 但缺少 `AICOService.Write`
- **WHEN** 收藏列表页面渲染
- **THEN** 收藏浏览、搜索、日期过滤、分页、展开正文和会话定位跳转 SHALL 保持可用
...
- **变更类型**：修改
- **目标内容**：Agent Web 收藏列表页面的写操作入口（单个取消收藏、批量取消收藏）在远程只读用户下被禁用并展示权限提示；收藏浏览等只读操作保持可用。
- **依据 Requirements**：`Write operation entries SHALL be disabled when user lacks AICOService.Write`

### 结果

- **变更类型**：修改
- **目标内容**：远程只读用户无法在收藏列表页面触发取消收藏写操作；有 Write 权限和 local 模式行为不变。
- **依据 Requirements**：`Write operation entries SHALL be disabled when user lacks AICOService.Write`
