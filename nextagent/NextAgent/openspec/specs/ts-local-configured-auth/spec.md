# ts-local-configured-auth Specification

## Purpose
定义 localhost local product entrypoint 的本地配置认证能力：启动时从冻结 app configuration 和 secret validation result 装配 local auth，提供 login/logout、signed HttpOnly cookie、未认证 challenge、可信 `IdentityContext` 注入和安全诊断，同时禁止 remote/IAM entrypoint 或非 localhost 暴露静默启用该能力。
## Requirements
...
### Requirement: IR Route Classification And Auth Isolation

When the IR surface is registered under `/api/v1/ir`, route classification SHALL direct `/api/v1/ir/**` requests to a header-based identity resolver (`createTaskIdentityResolver`) and SHALL NOT apply local cookie auth to those routes. ER routes under `/api/v1/**` (excluding the IR prefix) SHALL continue to use local cookie auth and SHALL NOT accept header-based identity.

The two authentication paths SHALL NOT cross-accept. An IR request without required headers SHALL be rejected before any runtime invocation. An ER request without a valid cookie SHALL be rejected before any runtime invocation. Neither rejection SHALL produce durable side effects.

#### Scenario: IR routes bypass cookie auth
- **WHEN** a request arrives at `/api/v1/ir/**` with valid identity headers but no cookie
- **THEN** the header-based resolver MUST produce a trusted IdentityContext
- **AND** local cookie auth MUST NOT be applied

#### Scenario: ER routes reject header-only auth
- **WHEN** a request arrives at an ER `/api/v1/**` route with identity headers but no valid cookie
- **THEN** local cookie auth MUST reject the request
- **AND** the header-based resolver MUST NOT be used for ER routes

#### Scenario: Missing IR headers are rejected before runtime
- **WHEN** a request arrives at `/api/v1/ir/**` without `x-tenant-id` or `x-subject-id`
- **THEN** the channel MUST return a safe 401
- **AND** MUST NOT invoke any runtime port or create any durable state
