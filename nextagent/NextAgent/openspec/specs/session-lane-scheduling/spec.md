# session-lane-scheduling Specification

## Purpose
定义 NextAgent TS runtime 的 same-session lane scheduling 规则：每个 `tenantId + subjectId + agentId + sessionId` 组合都是独立 session lane，Runtime 拥有该 lane 的 request acceptance、queueing、scheduler dispatch、execution status advancement、supersession 和 terminal commit。
