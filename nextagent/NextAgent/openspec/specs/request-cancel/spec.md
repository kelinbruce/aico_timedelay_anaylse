# request-cancel Specification

## Purpose
定义 NextAgent TS runtime 中用户发起的请求取消能力：取消命令必须通过 Runtime command boundary 进入，作用于可信 agent+owner scoped session lane 的当前最新可取消请求，并通过 Runtime 拥有的终态提交路径形成唯一、可审计、可恢复的取消事实。
...
- **AND** 终端 content MUST NOT 因 cancel 发生执行阶段不同而变化
