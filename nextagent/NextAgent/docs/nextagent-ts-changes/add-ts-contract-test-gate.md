...
状态：ready
类型：发布门槛 change
主要 owner：`agent-contracts`、各能力组 owner
依赖：首版必需 change 完成

目标：
- 为核心契约、channel event、capability descriptor、gateway store contract 建立必须通过的 contract test gate。
- 维护唯一标准命令 `npm run test:contract`；`harden-ts-local-runtime-release` 直接使用其退出状态，命令可按 `NEXTAGENT_RELEASE_CHECK_DIR` 协议补充安全报告，但不得要求 release-only 检查实现。
...
后续维护：
- 本 change 负责 contract 检查内容、标准命令和报告；调用顺序与 verdict 聚合归 `harden-ts-local-runtime-release`。
