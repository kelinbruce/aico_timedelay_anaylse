...
- **变更类型**：修改
- **目标内容**：系统在最新轮次 Edit 确认时区分未变化输入与有效变化；未变化时保留编辑状态和原轮次，有效变化时执行既有完整 replacement。
- **依据 Requirements**：`Agent Web SHALL expose edit only for the current latest turn`

### 结果

- **变更类型**：修改
- **目标内容**：未变化 Edit 不产生新 request、Retry attempt 或可见性副作用，并向用户提示内容未修改；文本或 Skill 定向变化时 Edit 正常提交。
- **依据 Requirements**：`Agent Web SHALL expose edit only for the current latest turn`
