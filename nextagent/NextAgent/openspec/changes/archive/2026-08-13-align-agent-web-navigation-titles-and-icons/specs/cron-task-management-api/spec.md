...
- **变更类型**：修改
- **目标内容**：系统通过 Cron task management API 管理当前可信范围内的定时任务，并在统一 Agent Web 页面中以当前语言下的定时任务页面名称展示任务看板；任务卡片继续展示创建人并保持单一更多操作菜单。
- **依据 Requirements**：`Cron task dashboard lists manageable tasks`

### 遗留规格

- **变更类型**：修改
- **目标内容**：`agent-web-cron-task-dashboard` 继续承载未触及的 Cron 看板 legacy Requirements；`Cron task dashboard lists manageable tasks` 迁入主规格 `cron-task-management-api`。
- **依据 Requirements**：`Cron task dashboard lists manageable tasks`
