...
状态：ready
类型：实施 change
主要 owner：`agent-capability`、gateway local port
依赖：`add-ts-capability-core-governance`、`add-ts-skill-manifest-contract`

目标：
- 支持本地 Skill source 目录加载：source 目录下每个一级子目录是一个 Skill，Skill 内部格式遵循统一 Skill manifest contract。
- 支持 `agents/{agentId}/skills/` 作为 Agent-scoped Skill source；`skills/` 下每个一级子目录是一个 Skill，并受该 Agent 作用域约束。
