# favorite-turn-list Specification

## Purpose

定义 Local、Immersive 和 Collaborative/PIU 宿主中收藏内容视图的交互行为：收藏入口导航到 `#/favorites` 并在主内容区域展示按 session 分组的分页收藏内容，左侧最近会话列表保持可见；收藏专用 URL 支持直达、刷新和浏览器前进/后退恢复；收藏使用统一 Agent Web Header 且不提供页内返回，离开收藏继续由宿主导航或扩展容器关闭入口决定；关键词搜索和精确到秒的起止时间过滤由服务端 `GET /api/v1/favorites` 在可信 scope 内先过滤再分页；同一 session 的收藏 turn 收敛为一张可展开的会话分组卡片，展开时复用既有会话读取契约展示 USER/ASSISTANT 正文并复用共享 Markdown 渲染；回答/问题 Tab 通过 `favoriteType` 参数切换；组内取消收藏经确认后按当前 Tab 写入；主内容入口选择互不耦合；Collaborative/PIU 从既有更多菜单在与记忆管理相同的左侧扩展内容容器复用同一收藏面板。
...
收藏主内容 MUST 使用 `agent-web-page-layout` 定义的 Header，并 MUST 从 Header 左侧内容起始位置展示当前语言下的收藏列表页面名称，MUST NOT 在标题前展示页内返回操作。收藏列表页面名称 MUST 由 `agent-web-page-layout` 的 `内置业务页面的导航标识与页面标题保持一致` Requirement 定义；本 Requirement MUST NOT 为收藏列表建立第二套页面名称。收藏 Header 的高度、水平内边距、标题字体、主题和窄视口行为 MUST 由 `agent-web-page-layout` 唯一定义，收藏页面 MUST NOT 再覆盖这些视觉规格。离开收藏内容、切换路由或关闭 PIU 左侧扩展内容容器 MUST 继续由当前宿主已有的导航或容器关闭入口决定，收藏页面 MUST NOT 新增平行导航 authority。
...
系统 MUST 将当前过滤结果中 `sessionId` 相同的收藏 turn 投影到同一个会话分组卡片，并 MUST 以 `sessionId` 而不是可重复的会话标题判定分组；不同 `sessionId` 的收藏 turn MUST NOT 合并。收藏内容、过滤区、列表和卡片 MUST 占满 `agent-web-page-layout` 产生的 `fluid` Content 可用宽度，MUST NOT 在该 Content 内增加固定最大宽度或重复水平安全内边距。收起状态的会话分组卡片最大高度 MUST 为 56px，左右内边距 MUST 为 16px，圆角半径 MUST 为 1rem，并依次展示展开按钮、会话标题或稳定 session 标识回退值和 `MM-DD HH:mm` 格式的最近收藏时间。相邻会话分组卡片的垂直间距 MUST 为 8px。展开按钮与摘要间距 MUST 为 8px，二者 MUST 左对齐，时间 MUST 右对齐。分组顺序 MUST 由各分组在收藏查询顺序中的首次出现位置决定；分组内 turn MUST 保持收藏查询返回的相对顺序。
...
#### Scenario: 收藏页面使用统一 Header
- **WHEN** 用户打开收藏内容视图
- **THEN** 收藏主内容 MUST 使用 `agent-web-page-layout` 定义的 Header
- **AND** Header MUST 从左侧内容起始位置展示当前语言下的收藏列表页面名称
- **AND** Header MUST NOT 展示页内返回操作
- **AND** 收藏页面 MUST NOT 覆盖统一 Header 的高度、水平内边距、标题字体、主题或窄视口行为
