# workflow-restful-sse Specification

## Purpose

定义 Workflow RESTFUL 节点的 SSE 流式执行模式：`stream_type: "sse"` 经 CLIP `subscribe` 原语发起流式调用，中间事件实时上报、聚合结果持久化并传递下游，与批处理/长轮询互斥，非 SSE 模式行为不变。
