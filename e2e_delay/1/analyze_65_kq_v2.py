"""
分析 nextagent-operational 日志 - knowledge-qa skill 性能 (v2)
适配新日志格式: timestamp带+08:00后缀, RAG事件名变更, RAG请求体结构变更
支持两种评测集: 无线(Col 72) / 云核(Col 71) - 通过命令行参数选择

用法:
  python analyze_65_kq_v2.py <日志目录> <场景>
  例: python analyze_65_kq_v2.py "D:/temp/20260717/AF2.0性能/71.77.65.24_0805_无线500题" wireless
  例: python analyze_65_kq_v2.py "D:/temp/20260717/AF2.0性能/71.77.65.24_0801晚_云核100题" cloud
"""
import json
import os
import re
...
for sid in sorted(kq_session_ids):
    ...
    for e in events:
        if e['event'] == 'capability.started':
            cap_started[e['capabilityInvocationId']] = e
        elif e['event'] == 'capability.completed':
            start_e = cap_started.get(e['capabilityInvocationId'])
            cap_id = start_e['details'].get('capabilityId', 'Unknown') if start_e else 'Unknown'
            dur = None
            if start_e and start_e['timestamp'] and e['timestamp']:
                dur = round((e['timestamp'] - start_e['timestamp']).total_seconds(), 3)
            elif e.get('durationMs') is not None:
    ...
    # Rebuild model_started_pending for step timeline
...
for sid in kq_session_ids:
    ...
    for e in events:
        if e['event'] == 'capability.started':
            cap_started_local[e['capabilityInvocationId']] = e
        elif e['event'] == 'capability.completed':
            start_e = cap_started_local.get(e['capabilityInvocationId'])
            cap_id = start_e['details'].get('capabilityId', 'Unknown') if start_e else 'Unknown'
            dur = None
            if start_e and start_e['timestamp'] and e['timestamp']:
                dur = round((e['timestamp'] - start_e['timestamp']).total_seconds(), 3)
