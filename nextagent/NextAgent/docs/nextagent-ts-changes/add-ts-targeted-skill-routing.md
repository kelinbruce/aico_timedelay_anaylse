...
类型：实施 change
主要 owner：`agent-core`
依赖：`add-ts-agent-routing-core`、`add-ts-capability-core-governance`、`refine-ts-routing-constraints-contract`、`add-ts-routing-constraint-validation`

目标：
- 支持用户显式指定技能，并继续经过可用性、权限和 owner scope 校验。
