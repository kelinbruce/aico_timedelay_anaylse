...
- **变更类型**：修改
- **目标内容**：系统按 `stepId` 隔离进行中累计正文，按真实时序显示 Tool 轮次的公开执行说明，并将终止模型轮次的完整安全回答作为唯一最终答案；live 与 history 不跨 step 或跨区域重复先前说明。
- **依据 Requirements**：`Tool 轮次执行说明与 Tool 调用连续呈现`

### 输出

- **变更类型**：修改
- **目标内容**：每个 `stepId` 输出自身累计公开正文，过程区输出每个 Tool 轮次说明一次，最终答案区只输出终止模型轮次的完整安全回答。
- **依据 Requirements**：`Tool 轮次执行说明与 Tool 调用连续呈现`

### 结果

- **变更类型**：修改
- **目标内容**：用户在 live 中看到按 `stepId` 隔离的累计正文，并在刷新后的 history 中看到一致且不重复的过程说明和最终答案。
- **依据 Requirements**：`Tool 轮次执行说明与 Tool 调用连续呈现`
...
- **变更类型**：新增
- **原规格值**：不适用（新增）
- **目标规格值**：非终态 model 正文只在同一 `stepId` 内累计；先前 Tool 轮次说明只在过程区显示一次；最终 Assistant Message 只包含终止模型轮次的完整安全回答。
- **依据 Requirements**：`Tool 轮次执行说明与 Tool 调用连续呈现`
