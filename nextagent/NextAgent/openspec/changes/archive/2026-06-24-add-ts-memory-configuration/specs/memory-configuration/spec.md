...
- `nextAgent` 对象允许只包含已定义的 `system` group、只包含已定义的 `memory` group，或同时包含二者；缺失 `nextAgent.memory` 等价于 memory 默认 disabled，不要求在默认配置文件中显式写入 `nextAgent.memory.enabled=false`。
...
4. 若 `nextAgent.memory.enabled` 缺失或为 `false`，配置状态为 `DISABLED`，memory consumers 必须看到稳定 disabled 快照。
5. 若配置有效，则填充默认值并冻结快照；冻结后同一进程内不得热更新。
...
#### Scenario: Memory disabled snapshot
- **WHEN** `nextAgent.memory.enabled` 缺失或为 `false`
- **THEN** 系统 MUST 生成状态为 `DISABLED` 的 `MemoryConfig` 快照
- **AND** app-composed selected memory port / core consumer boundary MUST 按 `add-ts-memory-core` 的 disabled 契约返回 `LTM_DISABLED`
- **AND** memory consumers MUST 不得把 disabled 伪装为成功空结果

### Requirement: Memory configuration namespace and defaults

系统 SHALL 定义 `nextAgent.memory.*` 外部配置命名空间。本 change 作为命名空间注册中心和校验入口，定义全局字段（`enabled`、`search.*`）并汇总已由对应 memory change 明确定义且获准进入实施范围的配置字段。后续新增 memory change 的配置字段 MUST 通过显式 spec delta 增加。
...
**配置契约**：
- `nextAgent.memory.enabled`：默认 `false`，允许值 `true|false`。
...
5. 任一未定义的 memory 配置字段必须显式拒绝或由 app configuration 的兼容策略标记为不生效，不得被某个 consumer 私自解释。

**状态 / 产物契约**：
- `search.default-limit` 和 `search.min-confidence` 只是默认查询参数；搜索排序公式和 owner scope 语义由 memory core 规格定义。
- 配置快照不表达 extraction、aging、maintenance、sharing 的状态机、调度计划、保留策略、共享策略、策略资源路径或写入策略。

#### Scenario: Defaults are applied for enabled configuration
- **WHEN** 源配置设置 `nextAgent.memory.enabled=true` 且省略 search 字段
- **THEN** `MemoryConfig` MUST 使用 search 字段默认值
- **AND** 配置状态 MUST 为 `VALID`

#### Scenario: Memory is disabled by default
...
- **THEN** app config schema validation MUST continue to accept the configuration
- **AND** `MemoryConfig` MUST be `DISABLED`
...
8. extraction prompt 未在本 change 中解释；fallback 和 unsupported language 诊断由 `add-ts-memory-extraction` 定义。
9. memory disabled：`nextAgent.memory.enabled` 缺失或为 `false` 时，配置状态 `DISABLED`，app-composed selected memory port / core consumer boundary 按 `LTM_DISABLED` 行为处理。
...
#### Scenario: Disabled memory is explicit or default degradation
- **WHEN** `nextAgent.memory.enabled` 缺失或为 `false`
- **THEN** memory configuration 状态 MUST 为 `DISABLED`
- **AND** health 或配置诊断 MUST 能显示 memory disabled 来自默认值或显式配置
