...
依赖：`add-ts-context-history-selection`、`add-ts-agent-package-assembly`、`add-ts-large-content-references`、`add-ts-context-compression`、`add-ts-traceable-summary-generation`、`add-ts-capability-core-governance`

目标：
- 基于 Agent assembly 中的 `promptTemplateIds`、`modelProfileIds`、`runtimeSettings.defaultPromptTemplateId` 和 `runtimeSettings.defaultModelProfileId` 选择 prompt template/profile，组装模型输入、系统提示、locale metadata 和 capability disclosure；具体模板内容来自配置、实现默认值或 Agent `prompts/` 目录，不在 change 中固化，也不内嵌在 `AgentAssembly` 中。
...
- `AgentAssembly` 只提供可选 profile/template id 集合和默认 id，不提供 prompt template 内容或 model profile 详情；context engine 必须通过对应配置/loader 获取内容并完成治理校验。
- Context Engine 必须通过 `AgentAssemblyRegistry.require(agentId, agentVersion)` 或由它派生的 assembly-scoped wrapper 获取当前 run 的 resolved assembly；不得解析 Agent package，也不得调用 `active(agentId)` 重新选择版本。
- `ContextAssembly` 是 render 输入，不保存最终 `ModelMessage[]`；最终模型消息在 render 后生成。
- `ContextAssembly` 保留 `SystemPrompt`、`selectedMessageRefs: MessageId[]`、`visibleCapabilities: CapabilityDescriptor[]`、`modelInfo`、`modelOptions` 和 `modelSelectionReason`，不新增 `ContextMessage`、`CapabilityDisclosure` 或完整 `modelProfile` 字段。
- `selectedMessageRefs` 在 render 时从不可变 message store/cache 读取；render 产出的 `RenderedModelInput` 只保留 request/session/run/message/step 最小执行坐标和模型调用所需字段。
- context selection、capability visibility、model selection reason、omitted/compaction reason 等审计或诊断信息在 assembly/render 生成时记录到 timeline、audit event、structured log 或 observability metric，不通过 `RenderedModelInput` 传递。
...
- 本 change 需要补充 `agent-contracts/context` public contract（section source/order、cache marker、typed source 标识、PromptTemplate 类型和变量白名单）；确认结论为可在本 implementation change 内作为 prompt shaping 最小 contract delta 落地。若实施发现需要新增 subpath、变更 owner、暴露 diagnostics 或重定义 `ModelOptions`，必须先提出 contract refinement change。
