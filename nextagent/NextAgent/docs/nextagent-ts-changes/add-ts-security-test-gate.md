...
状态：ready
类型：发布门槛 change
主要 owner：`agent-observability`、`agent-capability`、`agent-app`
依赖：首版必需 change 完成

目标：
- 验证 secret/redaction、sandbox deny-by-default、授权/高危确认、敏感日志泄露等安全边界。
- 本 change 主要拥有低层 contract、negative case 和 architecture/security assertion；真实 process/network/filesystem/sink 链路由 `add-ts-e2e-security-gate` 拥有。
- 维护唯一标准命令 `npm run test:gate:security`，纳入本 change 低层安全检查与 `add-ts-e2e-security-gate` 权威结果，并按 `NEXTAGENT_RELEASE_CHECK_DIR` 协议写出 machine-readable `ReleaseCheckResult`；任一必需组成结果非 `PASSED` 时不得返回 `PASSED`。
...
后续维护：
- 标准命令或有效报告未交付前，release qualification 必须将该检查归一化为 `MISSING` 并阻断；不得由 harden change 内置替代安全检查。
