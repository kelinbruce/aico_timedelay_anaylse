...
状态：ready
类型：实施 change
主要 owner：`agent-platform-gateway-local`、`agent-runtime`
依赖：`ship-ts-minimal-agent-kernel`

目标：
- 通过 `CheckpointStoreGateway` 将最小内核 no-op checkpoint 替换为真实本地 checkpoint 保存，并支持 idempotencyKey 写入、runVersion 冲突检测和按 sessionId/rootMessageId/runId 查询。
...
- Checkpoint payload 保存 `flowVariables` 和恢复所需坐标，但不保存 `attempt`、`deadlineAt` 或 `messageRefs`；`attempt`、`deadlineAt` 来自 `RequestRun`，当前 request/run 消息从 message store 查询。
- Checkpoint payload 不保存完整 `toolCallStates`；恢复时用 checkpoint 的 `runVersion`、`triggerReason`、`lastSequence`、`activeContextVersion` 校验 persisted messages 和 active context view，并从 assistant tool-use message 与 capability result messages 重建 tool batch state。
