...
- **变更类型**：修改
- **目标内容**：Workflow inner lifecycle 与产品过程由 Event 持有；terminal answer 与 Workflow-as-Tool outer protocol 继续由 Message 持有。
- **依据 Requirements**：`Workflow 内部过程与模型会话事实分离`、`Workflow 完成态产品过程可从 Event 恢复`

### 输入

- **变更类型**：修改
- **目标内容**：可信 Direct Workflow 或 governed Workflow Tool invocation 产生的 lifecycle、live-only fragment、completed product 与 terminal Assistant Message。
- **依据 Requirements**：`Workflow 内部过程与模型会话事实分离`、`Workflow 完成态产品过程可从 Event 恢复`

### 输出

- **变更类型**：修改
- **目标内容**：body-free lifecycle Event、durable completed product Event、独立 terminal conversation answer 与 outer model protocol Message。
- **依据 Requirements**：`Workflow 内部过程与模型会话事实分离`、`Workflow 完成态产品过程可从 Event 恢复`

### 处理过程

- **变更类型**：修改
- **目标内容**：按产品过程、模型协议与最终回答三类可观察语义选择唯一 durable owner；message-free 例外只适用于可信 Workflow inner lifecycle/product。
- **依据 Requirements**：`Workflow 内部过程与模型会话事实分离`、`Workflow 完成态产品过程可从 Event 恢复`

### 结果

- **变更类型**：修改
- **目标内容**：Direct 与 Workflow-as-Tool inner process 使用同一 Event contract，ordinary Message-backed process 与 terminal answer 的既有 Message owner 保持不变。
- **依据 Requirements**：`Workflow 内部过程与模型会话事实分离`、`Workflow 完成态产品过程可从 Event 恢复`

### 规格

- **规格项**：Workflow 产品过程与回答的 durable owner
- **变更类型**：新增
- **原规格值**：不适用（新增）
- **目标规格值**：inner lifecycle/product 使用 Event；terminal answer 与 Workflow-as-Tool outer protocol 使用 Message
- **依据 Requirements**：`Workflow 内部过程与模型会话事实分离`

- **规格项**：message-free 适用范围
- **变更类型**：新增
- **原规格值**：不适用（新增）
- **目标规格值**：仅可信 Direct Workflow 或 governed Workflow Tool invocation 的 inner lifecycle/product
- **依据 Requirements**：`Workflow 完成态产品过程可从 Event 恢复`

### 主规格

- **变更类型**：修改
- **目标内容**：`workflow-event-history`
- **依据 Requirements**：`Workflow 内部过程与模型会话事实分离`、`Workflow 完成态产品过程可从 Event 恢复`
