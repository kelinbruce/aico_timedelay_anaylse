...
Activity connection MUST 遵守 `refine-ts-session-activity-stream-boundary` 冻结的Session Activity Projection Stream边界：MUST NOT使用`StreamEnvelope`、`StreamEventType`、timeline sequence、`lastSeenSequence`、request/run filter或`RuntimeSessionPort.streamEvents(...)`。当前session detail connection继续属于Request Execution Stream；两类连接的建立、关闭、重连和失败MUST互不驱动。
...
- **变更类型**：新增
- **目标内容**：`FN-1.21 感知跨会话活动`
- **依据 Requirements**：`会话活动状态具有唯一语义和固定优先级`

### 描述

- **变更类型**：新增
- **目标内容**：系统持续向浏览器提供可信 Owner + Agent 范围内各会话当前需要用户注意的活动状态，使用户无需逐个打开会话即可识别运行、等待输入、未读失败和未读结果。
- **依据 Requirements**：`会话活动状态具有唯一语义和固定优先级`、`每个 app instance 使用一条全 scope 会话活动连接`、`会话列表按用户注意力优先级呈现活动`

### 前置条件

- **变更类型**：新增
- **目标内容**：浏览器已通过可信 channel/auth boundary 获得 Owner Scope，通过 app composition、hosted-agent selection 或已持久化 session 获得 Agent Scope；目标宿主使用受支持的 local、immersive 或 collaborative conversation surface。
- **依据 Requirements**：`每个 app instance 使用一条全 scope 会话活动连接`、`会话活动保持 Owner Scope 与 Agent Scope 隔离`

### 输入

- **变更类型**：新增
- **目标内容**：已提交的 session、run、pending-input 和 terminal facts，当前 app instance 的可信 scope，以及终态消费时的 `sessionId`、opaque `activityId` 与 `observedRunId`。
- **依据 Requirements**：`会话活动变化只由已提交 canonical facts 触发`、`终态活动仅在真实前台查看后匹配消费`、`会话活动保持 Owner Scope 与 Agent Scope 隔离`

### 输出

- **变更类型**：新增
- **目标内容**：首次连接或重连输出全部当前非 `NONE` 状态的稀疏 snapshot，之后输出 session-keyed delta；会话列表输出单一活动 marker，collaborative History 输出是否存在后台活动的聚合蓝点；合法终态消费输出匹配清除结果。
- **依据 Requirements**：`会话活动公共数据契约是严格判别联合`、`首帧稀疏快照与后续 delta 不丢失状态`、`会话列表按用户注意力优先级呈现活动`、`Collaborative History入口聚合不在当前查看表面的会话活动`

### 处理过程

- **变更类型**：新增
- **目标内容**：系统从已提交事实按固定优先级派生每个 session 的唯一状态，通过独立全 scope 连接同步 snapshot/delta，在当前 conversation surface 本地抑制 marker，并仅在匹配终态结果真实前台可见后消费未读活动；删除、重启、依赖失败和连接重建按显式降级边界处理。
- **依据 Requirements**：`会话活动状态具有唯一语义和固定优先级`、`会话活动变化只由已提交 canonical facts 触发`、`终态活动仅在真实前台查看后匹配消费`、`会话删除和离线期间遵守显式活动生命周期`、`会话活动在重启和依赖失败后安全恢复`

### 结果

- **变更类型**：新增
- **目标内容**：正常时三个宿主一致呈现当前 scope 的会话活动；重复、迟到或跨 scope 消费不会清除较新或未授权活动；协议、bootstrap、consume 或投影失败不改变 request lifecycle，并通过重连 snapshot 最终校正。
- **依据 Requirements**：`终态活动仅在真实前台查看后匹配消费`、`会话活动在重启和依赖失败后安全恢复`、`会话活动保持 Owner Scope 与 Agent Scope 隔离`

### 量化指标

- **指标名称**：单 app instance 活动连接数
- **变更类型**：新增
- **原值或原口径**：不适用（新增 Function）。
- **目标值或目标口径**：按配置只建立一条 Owner + Agent 全 scope Activity connection；SSE 与 WebSocket 不并行建立。
- **单位与测量边界**：条/浏览器 app instance；从 runtime bootstrap 完成到 app instance 销毁。
- **依据 Requirements**：`每个 app instance 使用一条全 scope 会话活动连接`

- **指标名称**：单 session 活动状态数
- **变更类型**：新增
- **原值或原口径**：不适用（新增 Function）。
- **目标值或目标口径**：每个 session 至多一个内部 activity 状态；公开状态与消费抑制状态互斥。
- **单位与测量边界**：个/session；按同一 Owner + Agent + Session coordinate 计数。
- **依据 Requirements**：`会话活动状态具有唯一语义和固定优先级`、`会话活动状态和待发送变化保持有界`

- **指标名称**：scope 与 subscriber 活动容量
- **变更类型**：新增
- **原值或原口径**：不适用（新增 Function）。
- **目标值或目标口径**：内部状态数量和待发送 session 数量均不超过同一 Owner + Agent scope 的实际 session 数 `N`，不得以更小 quota 静默丢弃。
- **单位与测量边界**：个 session/scope 或 subscriber；以当前实际 session 集合为计数边界。
- **依据 Requirements**：`会话活动状态和待发送变化保持有界`

### 接口

- **变更类型**：新增
- **目标内容**：ER-only `GET /api/v1/session-activities/stream`、`WS /api/v1/session-activities/ws` 与 `POST /api/v1/sessions/:sessionId/activity/consume`；SSE/WS 按 `transportKind` 互斥选择。
- **依据 Requirements**：`每个 app instance 使用一条全 scope 会话活动连接`、`终态活动仅在真实前台查看后匹配消费`

### 覆盖特性

- **变更类型**：新增
- **目标内容**：`F-2.4 查看请求状态`
- **依据 Requirements**：`会话列表按用户注意力优先级呈现活动`、`Collaborative History入口聚合不在当前查看表面的会话活动`

### 主规格

- **变更类型**：新增
- **目标内容**：`cross-session-activity-awareness`
- **依据 Requirements**：本 change 全部 ADDED Requirements
