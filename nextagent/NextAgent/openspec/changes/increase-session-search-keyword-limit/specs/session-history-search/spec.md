...
- **WHEN** 用户在会话列表搜索框输入 trim 后超过 200 个 Unicode code point 的关键词
...
#### Scenario: 非法查询参数失败关闭
- **WHEN** 客户端提供超过 200 个 Unicode code point 的 `q`、非整数或非有限时间戳、超过 90 天减 1 毫秒的创建时间范围、负数 `offset` 或超过 50 的搜索 `limit`
...
- **变更类型**：修改
- **目标内容**：会话列表关键词搜索支持 trim 后非空且不超过 200 个 Unicode code point 的 `q`；超过上限时前端不发起请求，Web API 失败关闭。
- **依据 Requirements**：`前端会话列表复用原展示并提供搜索和日期交互`、`搜索查询保持 scope 隔离和安全校验`

### 规格

- **规格项**：搜索词长度
- **变更类型**：修改
- **原规格值**：trim 后非空且不超过 50 个 Unicode code point
- **目标规格值**：trim 后非空且不超过 200 个 Unicode code point
- **依据 Requirements**：`前端会话列表复用原展示并提供搜索和日期交互`、`搜索查询保持 scope 隔离和安全校验`

### 主规格

- **变更类型**：修改
- **目标内容**：`session-history-search`
- **依据 Requirements**：`前端会话列表复用原展示并提供搜索和日期交互`、`搜索查询保持 scope 隔离和安全校验`