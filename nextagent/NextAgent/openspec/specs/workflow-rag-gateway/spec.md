# workflow-rag-gateway Specification
...
remote gateway MUST NOT 过滤、改写或丢弃 per-index 参数。mapping 是 HTTP transport 层关注点，contract 层（`agent-contracts/gateway`）保持 provider-neutral 命名 `indexes`/`indexName`，与 `RagRetrievalRequest.indexes` 同形同策；平台 wire format `ragIndexes`/`ragIndex` 不进入 contract。
