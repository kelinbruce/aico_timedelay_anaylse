...
系统 SHALL 把本地 Skill source 的 provider identity 作为 trusted composition owned identity 管理。`local-skills-system` SHALL 表示 system-level local Skill provider，`local-skills-agent-owned` SHALL 表示 Agent-owned local Skill provider。两者都 MUST 使用 `providerKind=LOCAL_DIRECTORY`，并且只能由 app composition / resource provider registry 注册。
...
Built-in provider identity MUST be owned by app composition, not by `default-system.yaml`. `builtin-tools`, `builtin-skills`, `local-skills-system`, and `local-skills-agent-owned` SHALL be registered by the startup resource provider registry. `default-system.yaml` and user `application.yaml` MUST NOT use a raw `capabilityProviders.providers` entry to declare `builtin-tools` or any other framework/reserved provider.

#### Scenario: Reserved provider identity 由 app composition 注册

- **WHEN** 系统启用本地 Skill source
- **THEN** app composition / resource provider registry 必须注册 `local-skills-system` 和 `local-skills-agent-owned` 这两个 trusted provider identity
...
- **AND** `builtin-tools` 和 `builtin-skills` 必须由 app composition 的 startup resource provider registry 注册
...
系统 SHALL 在可信 app config / app composition 中冻结两个 root：`configRoot` 和 `workspaceRoot`。`configRoot` 承载用户 `application.yaml`、`skills/` 和 `agents/`；`workspaceRoot` 承载运行数据、SQLite、logs 和 Agent execution workspace。`workspaceRoot` MUST come from the final system config after applying `application.yaml` over the internal `default-system.yaml`. `configRoot` MUST come from the `application.yaml` containing directory, or from the current startup base dir when no user application config is supplied.
...
`agentsRoot` 是 frozen derived path `configRoot/agents`，不属于 `CapabilityProviderConfig`，也不得由 `agent-capability` 直接从 raw app config 解释。`AgentAssembly.workspaceDir` 是执行 workspace / provider execution root，不得作为 Agent-owned Skill source root 的权威来源。
...
- **AND** Agent execution workspace 必须从 `workspaceRoot/{workspaceDir-or-agentId}` 派生
- **AND** diagnostics 不得暴露 raw absolute path

#### Scenario: workspaceDir 不作为 Agent-owned Skill root

- **WHEN** `AgentAssembly.workspaceDir` 与 `configRoot/agents/{agentId}` 指向不同目录
- **THEN** Agent-owned Skill discovery 必须使用 `configRoot/agents/{agentId}/skills`
- **AND** 系统不得扫描 `workspaceDir/skills` 作为 Agent-owned Skill source
...
- **AND** 系统不得扫描 `AgentAssembly.workspaceDir/skills`
...
- **AND** Agent-owned local Skill discovery 仍不得扫描 `workspaceDir/skills`
