...
Agent core MUST 依据 `incompleteOutputReason` 而不是 `finishReason` 进入唯一输出恢复流程，并先尝试一次同请求预算提升。`incompleteOutputReason="output-limit"` 的预算提升结果 content 为空、没有 Tool call 且携带非空 reasoning 时，Agent core MUST 在普通续写前注入一次 reasoning-only 收敛指令并重试；该指令 MUST 要求模型直接给出简洁可见回答或一次必要 Tool call，MUST NOT 要求或重复内部推理。该收敛重试在当前 model round 内 MUST 至多发生一次，且 MUST 只存在于 request-local 消息，不得作为独立 session message 持久化。收敛重试后仍为可安全续写的纯文本 `output-limit` 时，系统 MUST 在同一 request run 内最多发起 3 次 request-local 续写。`incompleteOutputReason="truncated-tool-call"` 只允许同请求预算提升重新生成一次；预算提升后仍有任一 `incompleteOutputReason` 时 MUST 安全失败，不得把残缺 Tool call 转成文本续写。只有恢复调用没有 `incompleteOutputReason`、没有 `safeError` 且满足既有 terminal output guard 时，系统才可提交最终回答或执行完整 Tool call。
...
#### Scenario: 提升预算后 reasoning-only 空产出先收敛再续写
- **GIVEN** 同请求预算提升后的结果仍携带 `incompleteOutputReason="output-limit"` 且 `finishReason="length"`
- **WHEN** content 为空、没有 Tool call 且 reasoning 非空
- **THEN** Agent core MUST 注入一次 reasoning-only 收敛指令后重试
- **AND** 该收敛重试在当前 model round 内 MUST 至多发生一次
- **AND** 若收敛重试仍为不完整输出，后续处理 MUST 继续遵循既有最多三次续写与失败边界

#### Scenario: 提升预算后最多续写三次
- **GIVEN** 同请求预算提升或收敛重试后的纯文本结果仍携带 `incompleteOutputReason="output-limit"`
...
- **WHEN** 提升预算或收敛重试后的结果与随后 3 次续写结果均携带 `incompleteOutputReason="output-limit"`
...
#### Scenario: 取消中止恢复链
- **WHEN** 当前 request 的 `AbortSignal` 在预算提升、reasoning-only 收敛重试或续写期间被取消
...
- **变更类型**：修改
- **目标内容**：空 Tool-call 终态只有在携带精确截断证据时进入恢复；预算提升后仍为 reasoning-only `length` 时先执行一次收敛重试，再进入既有有界续写。
...
|---|---|---|---|---|
| 输出 Token 恢复 | 修改 | 首次预算提升，仍超限时最多 3 次 request-local 续写 | 只对精确截断证据恢复；首次预算提升后若仍为 reasoning-only `length`，先做至多一次收敛重试，再执行最多 3 次 request-local 续写 | `Failure exits are explicit and safe`、`输出超限不得静默截断` |
