...
## Purpose
定义普通 Agent Tool loop 对 Capability 最终结果、显式生命周期控制、取消、单轮 Tool-call 接纳和 logical turn 上限的统一处置，使模型可以基于完整安全事实恢复或结束，并在预算耗尽后通过一次无 Tool 执行权的模型收尾确定结束。
...
- **spec 角色**：主规格
## Requirements
### Requirement: Tool loop projects in-flight capability result deltas without changing terminal semantics

Agent Core tool loop MUST support zero or more in-flight `CAPABILITY_RESULT_DELTA` projections emitted during one ordinary Tool invocation. These deltas MUST use the existing canonical stream event vocabulary and MUST NOT introduce a parallel capability progress event name.

Projecting in-flight capability result deltas MUST NOT change the existing terminal semantics: the tool loop MUST still emit `CAPABILITY_STARTED` before execution, MUST still append at most one terminal capability result message for the invocation, and MUST still emit exactly one terminal `CAPABILITY_COMPLETED` outcome for that invocation.

#### Scenario: In-flight deltas appear before completion

- **WHEN** a Tool invocation emits cumulative result deltas before it returns its final result
- **THEN** the tool loop MUST project those deltas as `CAPABILITY_RESULT_DELTA` events during the active invocation
- **AND** the invocation MUST remain in progress until the Tool returns its terminal result

#### Scenario: In-flight deltas do not create multiple terminal result messages

- **WHEN** a Tool invocation emits one or more in-flight result deltas
- **THEN** the tool loop MUST NOT append a persisted `CAPABILITY_RESULT` message for each delta
- **AND** it MUST append at most one terminal capability result message after the invocation finishes according to existing completion rules
- **AND** it MUST NOT emit more than one `CAPABILITY_COMPLETED` event for the same tool call
