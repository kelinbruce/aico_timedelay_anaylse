...
用户选择“我的记忆”或“已归档”列表中的记忆时，页面 SHALL 调用 GET `/{memoryId}/record` 并委托 `getLongTermMemory` 加载完整记录。管理界面查看详情 SHALL NOT 增加 `accessCount` 或改变 `lastAccessedAt`；这两个统计只由智能体实际使用记忆的链路维护。详情区 SHALL 显示摘要、内容、标签、置信度、类型、来源、状态、保持状态、访问次数、最近访问时间、时间戳和操作按钮。
...
- **THEN** 中文分别显示“访问次数”和“最近访问时间”
- **AND** 英文分别显示 “Access count” 和 “Last access time”
...
- **变更类型**：新增
- **目标内容**：系统为可信用户提供长期记忆的列表、搜索、详情、手工新增和编辑、状态管理、删除、共享发布、取消发布与复制能力，并在 immersive Shell 内保持一致的多宿主交互。
- **依据 Requirements**：`长期记忆管理必须复用 immersive NextAgent Shell`、`记忆列表必须支持筛选、搜索和分页`、`用户创建和编辑必须使用手工保存端点`、`共享记忆库必须支持浏览、搜索、发布、取消发布和复制`

### 前置条件

- **变更类型**：新增
- **目标内容**：用户已通过可信宿主与 Channel 身份边界形成 Owner Scope，应用已选择 Agent Scope 和记忆实例，长期记忆管理端口已完成装配。
- **依据 Requirements**：`路由边界必须从可信身份解析器获得身份字段`、`记忆管理端口必须由 composition root 完成装配`

### 输入

- **变更类型**：新增
- **目标内容**：可信范围内的列表、搜索和分页条件，手工记忆内容及类型、置信度和标签，状态变更、发布、取消发布、复制与删除操作；客户端输入不得包含或覆盖可信身份和 Agent 归属。
- **依据 Requirements**：`记忆列表必须支持筛选、搜索和分页`、`用户创建和编辑必须使用手工保存端点`、`PATCH 端点必须按字段组执行变更`、`路由边界必须从可信身份解析器获得身份字段`

### 输出

- **变更类型**：新增
- **目标内容**：界面输出三个记忆集合的未过滤总数、分页列表、管理详情与操作反馈；共享复制结果以 `COPIED | EXISTING` 区分新建和既有副本；摘要和正文的只读投影不暴露宿主绝对文件路径；Web API 输出公开 DTO 或安全错误，不泄漏 Gateway Record。
- **依据 Requirements**：`记忆管理布局必须适配 Shell 内容区`、`记忆详情必须展示智能体使用统计且不产生统计副作用`、`共享记忆库必须支持浏览、搜索、发布、取消发布和复制`、`记忆只读展示必须扩展 Chat 通用敏感内容保护并单独隐藏绝对路径`、`后端 HTTP 路由必须委托给记忆管理端口`

### 处理过程

- **变更类型**：新增
- **目标内容**：系统在可信 scope 内校验查询和写入输入，按服务端分页读取记忆并对只读摘要和正文执行绝对路径脱敏后投影详情；写操作按明确字段组执行，发布归属来自 IdentityContext，成功后刷新受影响列表与计数。
- **依据 Requirements**：`记忆列表必须支持筛选、搜索和分页`、`记忆只读展示必须扩展 Chat 通用敏感内容保护并单独隐藏绝对路径`、`用户创建和编辑必须使用手工保存端点`、`详情区共享按钮必须反映发布状态`、`PATCH 端点必须按字段组执行变更`

### 结果

- **变更类型**：新增
- **目标内容**：合法操作产生可核对的记忆状态或共享结果；新复制进入“我的记忆”第一页，重复复制只提示既有副本所在集合且不定位或恢复；非法输入、容量超限、内容安全拒绝、并发冲突和依赖失败以本地化反馈或安全错误结束，不允许跨 Owner Scope 或 Agent Scope 读取和写入。
- **依据 Requirements**：`用户创建和编辑必须使用手工保存端点`、`共享记忆库必须支持浏览、搜索、发布、取消发布和复制`、`SafeError 必须映射为 LtmError 和确定的 HTTP 状态`、`路由边界必须从可信身份解析器获得身份字段`、`已删除记忆的过期界面操作必须安全收敛`

### 接口

- **变更类型**：新增
- **目标内容**：immersive/PIU Shell 内的记忆管理内容区，以及 `/api/v1/memory/long-term-mem` 下由 `LongTermMemoryManagementPort` 承载的公开管理端点。
- **依据 Requirements**：`长期记忆管理必须复用 immersive NextAgent Shell`、`后端 HTTP 路由必须委托给记忆管理端口`

### 覆盖特性

- **变更类型**：修改
- **目标内容**：`F-8.2 长期记忆` 增加面向用户的长期记忆管理能力。
- **依据 Requirements**：本 change 全部 ADDED Requirements

### 主规格

- **变更类型**：新增
- **目标内容**：`long-memory-web-management`
- **依据 Requirements**：本 change 全部 ADDED Requirements
...
- **规格项**：手工个人设定记忆容量
- **变更类型**：新增
- **原规格值**：不适用（新增 Function）
- **目标规格值**：同一可信 Owner Scope、Agent Scope 和记忆实例下，ACTIVE 与 ARCHIVED 的 `CONFIGURED` 个人记忆合计最多 50 条
- **依据 Requirements**：`用户创建和编辑必须使用手工保存端点`

- **规格项**：搜索文本长度
- **变更类型**：新增
- **原规格值**：不适用（新增 Function）
- **目标规格值**：最多 128 个 Unicode code point
- **依据 Requirements**：`记忆列表必须支持筛选、搜索和分页`

- **规格项**：手工置信度
- **变更类型**：新增
- **原规格值**：不适用（新增 Function）
- **目标规格值**：`0`、`1` 或 `0..1` 内最多两位小数
- **依据 Requirements**：`用户创建和编辑必须使用手工保存端点`

- **规格项**：只读敏感内容保护
- **变更类型**：新增
- **原规格值**：不适用（新增 Function）
- **目标规格值**：摘要和正文复用 Chat 的凭据、Token、手机号与 Private Key 通用保护类别，并由记忆只读投影单独隐藏绝对路径；Chat 正文保留绝对路径，相对路径、URL 和 IP 地址保持原文
- **依据 Requirements**：`记忆只读展示必须扩展 Chat 通用敏感内容保护并单独隐藏绝对路径`
