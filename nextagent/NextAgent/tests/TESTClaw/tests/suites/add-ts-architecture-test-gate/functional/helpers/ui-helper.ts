...
export async function waitForStreamComplete(page: Page, timeoutMs = 30_000): Promise<void> {
  await page.locator('[data-testid="chat-stream-status-strip"]').waitFor({ state: 'visible', timeout: timeoutMs });
}
...
export async function waitForPendingInput(page: Page, timeoutMs = 30_000): Promise<void> {
  await page.locator('[data-testid="respond-input-panel"]').waitFor({ state: 'visible', timeout: timeoutMs });
}
...
export async function toggleTheme(page: Page): Promise<void> {
  // 前端未提供 theme-toggle-btn；深色模式通过 runtimeConfig 或 sidebar 控制
  // 暂用 sidebar 会话列表控制区域作为近似切换入口
  await page.locator('[data-testid="sidebar-session-list-controls"]').click();
}

/** 获取会话列表 UI 状态 */
export async function getSessionListState(page: Page): Promise<{ expanded: boolean; visibleCount: number }> {
  // TODO: 需要真实 UI 实现后补充
  const items = await page.locator('[data-testid^="sidebar-session-item"]').count();
  const expanded = await page.locator('[data-testid="sidebar-session-list"]').isVisible();
  return { expanded, visibleCount: items };
}

/** 切换到指定会话 */
export async function switchSession(page: Page, sessionId: string): Promise<void> {
  await page.locator(`[data-testid="sidebar-session-item-${sessionId}"]`).click();
}
