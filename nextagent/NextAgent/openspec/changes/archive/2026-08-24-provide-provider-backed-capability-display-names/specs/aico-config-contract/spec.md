...
AICOConfig SHALL 是用于定制 NextAgent 前端外观、行为开关、布局和 PIU 渲染注入点的 JSON-compatible 配置对象。全部字段均为 optional；整个 AICOConfig 或任一字段缺失时，对应界面或行为 MUST 使用当前默认值。未提供任何字段时，AICOConfig MUST NOT 引入行为变化。
...
既有 supporting types 的 shape 与语义保持不变。AICOConfig MUST NOT 定义或消费 Capability 业务名称、Capability identity 到名称的映射或其他 Capability descriptor 展示事实；这些事实 MUST 来自受治理的 Capability Provider descriptor。
...
- `operators` MUST 逐项校验；invalid item MUST 被过滤并产生 console warning，其他合法项继续生效。
...
validator MUST NOT 产生、保留或返回 Capability 名称映射。Unknown Capability 名称 key MUST 与其他 unknown field 一样静默忽略，MUST NOT 因其内容触发逐项校验或 warning。
...
#### Scenario: Unknown 名称字段不进入校验结果

- **GIVEN** AICOConfig 同时包含合法 `welcome` 和任意 `capabilityBusinessNames`
- **WHEN** 前端校验该对象
- **THEN** 合法 `welcome` MUST 被保留
- **AND** validator 结果 MUST NOT 包含 `capabilityBusinessNames`
- **AND** 前端 MUST NOT 为该 unknown field 产生 warning
...
AICOConfig MUST NOT 为 Capability 名称定义默认值或 fallback。Capability 名称缺失与降级 MUST 由 Capability 展示资源及其前端 resolver 契约处理。
...
- **变更类型**：修改
- **目标内容**：删除 `AICOConfig.capabilityBusinessNames` 及只服务该字段的 supporting types；三宿主注入接口不变。
- **依据 Requirements**：`AICOConfig configuration type and field definitions`

### 规格

- **规格项**：Capability 名称扩展入口
- **变更类型**：修改
- **原规格值**：AICOConfig 可提供最多 1000 项中英文 Capability 业务名称映射
- **目标规格值**：AICOConfig 不接受 Capability 名称；统一由 Provider descriptor 与安全展示资源提供
- **依据 Requirements**：`AICOConfig configuration type and field definitions`、`AICOConfig validation uses hand-written functions`、`AICOConfig default behavior when fields are absent`
