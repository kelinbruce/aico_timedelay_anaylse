...
目标：
- 定义 app composition 配置 schema、校验和 safe config error。
...
- Provider 配置采用稳定的 `modelProfiles[]` baseline：`profileId`、`providerKind`、`modelName`、`baseUrl`、`credentialRef`、`timeoutMs`、`enabled`、`fallbackEligible`。
- `profileId` 必须非空且唯一。
...
- 启用的 cloud provider 必须通过 `credentialRef` 提供凭据，不允许 raw API key 进入日志、trace、audit、stream、safe error 或模型上下文。
...
- Local auth 配置只承载 localhost-only local auth 所需的单一本地用户身份、credential source、cookie/ticket 基础参数和产品入口选择；页面修改认证配置、多用户管理、注册、密码修改、remember-me、refresh token 或服务端认证 session store 不属于本配置 change。

并行边界：
- 配置 schema 属于 app composition 边界。
- 完整 app configuration snapshot 保持在 `agent-app` 内部；本 change 不新增或改动 `agent-contracts` owning surface。
