...
状态：active
类型：实施 change
主要 owner：`agent-observability`、`agent-runtime`
依赖：`add-ts-structured-logging`

目标：
- 补实 request/run diagnostic context、event envelope snapshot 和日志/trace 关联。
