...
## Purpose
Defines `Skill` as the model-facing Tool entry for executing governed Skills by name. One model `Skill` tool_use produces exactly one correlated tool_result. Covers name resolution, inline execution with hidden context injection, safe acknowledgement, timeout governance, and Skill disclosure format.
...
Skill disclosure SHALL 由 builtin `SYSTEM_PROMPT` 模板的 builder-owned `skill_disclosure` section 承载：`### Available skills` 列表 SHALL 由 context engine 的安全投影变量提供（按 `kind="SKILL"`、availability、`modelInvocable=true` 和 disclosure policy 门控过滤后的 governed Skill names 与 safe descriptions），`### How to use skills` 指令正文 SHALL 来自模板内容。Agent MAY 通过既有 agent-over-builtin SYSTEM_PROMPT 覆盖机制定制 `skill_disclosure` section 内容；覆盖 MUST 继续受 Skill tool 可见性门控约束，MUST NOT 绕过 section 省略规则，覆盖内容 MAY 引用同一组安全投影变量。builtin 默认内容 MUST 保持英文并满足既有指令要求；Agent 覆盖内容的安全约束 SHALL 由 Agent package 治理承载。
...
#### Scenario: Skill disclosure 使用受治理模板 section 承载
- **WHEN** 当前 model step 有可见 `Skill` tool entry 和至少一个可见 model-invocable Skill
- **THEN** system prompt MUST 渲染 builtin `SYSTEM_PROMPT` 模板的 `skill_disclosure` section，heading 恰好为 `### Available skills` 和 `### How to use skills`
- **AND** `### Available skills` list MUST 来自安全投影变量，按 `- <skill-name>: <safe description>` 格式为每个 visible Skill 包含一个 bullet
- **AND** builtin 默认 `### How to use skills` 内容 MUST 指示模型为列表中的 Skills 调用 `Skill` tool、使用精确列出的 `name`、让 `args` 保持为 task-specific JSON、使用 governed names，并在没有列表 Skill 明确匹配时安全继续使用普通 tools
- **AND** builtin 默认内容 MUST 保持英文；`tool-search` 披露模式下 builtin 默认内容 MUST 指示通过 `ToolSearch` 发现未列出的 deferred Skills 并按 `defer_loading=true` 语义调用 `Skill` 加载
- **AND** section surface MUST 限于 headings、governed Skill names、safe descriptions、披露模式事实和 usage instructions
- **AND** Agent MAY 通过 agent-over-builtin SYSTEM_PROMPT 覆盖机制定制该 section 内容，覆盖内容 MAY 引用安全投影变量

#### Scenario: Agent 覆盖 skill_disclosure 仍受门控约束
- **WHEN** Agent template 覆盖了 `skill_disclosure` section，且当前 model step 没有可见 `Skill` tool entry 或没有满足披露门控的 Skill
- **THEN** system render policy MUST 过滤掉该 section，使其不出现在最终 system prompt 中
- **AND** 覆盖内容 MUST NOT 使被过滤的 section 出现
- **AND** 覆盖内容引用的 skill 列表投影 MUST 只包含满足门控的 governed Skill names 与 safe descriptions

#### Scenario: Skill disclosure 使用固定英文 prompt 格式
- **WHEN** 当前 model step 有可见 `Skill` tool entry 和至少一个可见 model-invocable Skill
- **THEN** Context Engine MUST 通过 builtin `SYSTEM_PROMPT` 模板的 `skill_disclosure` section 追加英文 system-prompt section，heading 恰好为 `### Available skills` 和 `### How to use skills`
...
### Requirement: Skill Tool Non-Agentic Dispatch

The `Skill` tool SHALL check `extension._naie_agentic_loop_flag` after `readSkillMetadata` and before the existing fork context check. When the flag is `"false"`, the `Skill` tool MUST still load the skill body via `loadCanonicalBodyView` to parse the api command, but MUST NOT proceed with resource projection or body injection. It MUST return a `CapabilityInvocationResult` containing skill identity, parsed api command info, and a `nonAgenticApiCall: true` signal in metadata. When the flag is `"true"` or absent, the existing execution path MUST remain completely unchanged.

#### Scenario: Flag false returns non-agentic result without body injection

- **WHEN** the model calls `Skill` with a valid skill name
- **AND** `readSkillMetadata` returns metadata with `extension._naie_agentic_loop_flag` equal to `"false"`
- **THEN** the `Skill` tool MUST call `loadCanonicalBodyView` to load the body for api command parsing
- **AND** MUST NOT call `projectSkillResources`
- **AND** MUST return `CapabilityInvocationResult` with `status=SUCCEEDED`
- **AND** `structuredPayload` MUST contain the resolved skill name and parsed api command from skill body
- **AND** `metadata` MUST contain `nonAgenticApiCall: true`
- **AND** `generatedMessages` MUST be empty

#### Scenario: Flag true or absent follows existing path

- **WHEN** the model calls `Skill` with a valid skill name
- **AND** `extension._naie_agentic_loop_flag` is `"true"` or the extension field is absent
- **THEN** the `Skill` tool MUST proceed with the existing inline body loading and injection path
