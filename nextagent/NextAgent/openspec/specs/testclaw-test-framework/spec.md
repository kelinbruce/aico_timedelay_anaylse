# testclaw-test-framework Specification

## Purpose
TESTClaw 定义针对解包后 NextAgent binary package 的独立黑盒验证框架。它在不依赖源码 workspace 测试 harness 的前提下，校验 package 布局、配置安全、命令运行目录约束、服务生命周期、E2E 可达性和报告生成。

## Requirements
### Requirement: TESTCLAW-DIR-STRUCTURE-001 目录结构

TESTClaw 测试框架 SHALL 组织在 `tests/TESTClaw/` 之下，并包含以下必需子目录：
- `target/`：被测产品目录，NextAgent binary package 解包到这里
...
### Requirement: TESTCLAW-TARGET-SETUP-002 target 目录配置

被测产品 SHALL 解包到 `target/` 目录，其中包含：
...
`scripts/setup-package.mjs` SHALL 自动检测 `target/package.json` 是否存在；不存在时，SHALL 自动创建一个只含 `name` 和 `version` 的最小 `package.json`。

#### Scenario: target 目录包含必需文件
- **WHEN** 用户把 NextAgent binary package 解包到 `target/` 目录
...
API 配置 SHALL 使用 `env:` 或 `file:` 引用；SHALL NOT 以明文直接写入 API Key。`target/config/default-system.json` 中的 `credentialRef` 字段 SHALL 遵守 secret-configuration-boundary 规则。
...
4. 校验 SHALL 在服务启动前执行；校验失败时，服务 SHALL NOT 启动。
...
### Requirement: TESTCLAW-CMD-RUNDIR-004 binary package 命令在 target/ 目录运行

binary package 命令 SHALL 在 `target/` 目录中执行，包括：
...
2. 如果 `package.json` 存在但缺少 `config/`，nextagent-self-check SHALL 返回 `invalid-config-sample` 错误。
...
### Requirement: TESTCLAW-E2E-SERVICE-005 E2E 测试要求服务在运行

Playwright E2E 测试 SHALL 在 NextAgent 服务运行于 `http://127.0.0.1:3000` 的状态下执行。统一 runner（`run-tests.ps1`）SHALL：
1. 在 E2E 测试前自动启动服务（除非提供 `-NoStart` 标志）
2. 在使用 `-NoStart` 模式时校验服务可达
3. 在 E2E 测试后自动停止服务（除非提供 `-KeepRunning` 标志）
...
- **THEN** 框架 SHALL 在写报告前自动创建该目录

### Requirement: TESTCLAW-GITIGNORE-007 测试产物不提交

以下 `tests/TESTClaw/` 下的目录 SHALL 在 `.gitignore` 中排除，并 SHALL NOT 提交到仓库：
...
- **THEN** `node_modules/`、`target/`、`data/`、`logs/`、`test-output/`、`docs/`、`.skills/` 中任何一个都 SHALL NOT 出现在暂存列表中
...
- 针对中文测试名的终端编码修复（chcp 65001，UTF8 控制台编码）
- 进度计时器（每 60 秒一次）
- 实时输出日志
- 耗时摘要

Vitest 使用 `npm.cmd run test`，Playwright 使用 `npm.cmd run test:e2e`。后端测试和 E2E 测试 SHALL 相互独立运行——Vitest 失败 SHALL NOT 阻塞 Playwright 执行。

#### Scenario: 自动启动的完整测试运行
- **WHEN** 用户在设置好 API 环境变量后运行 `.\scripts\run-tests.ps1 -All`
- **THEN** runner SHALL：执行 self-check、启动服务、运行 Vitest、运行 Playwright、停止服务、打印摘要

#### Scenario: 服务已运行时仅执行 E2E
- **WHEN** 用户运行 `.\scripts\run-tests.ps1 -E2E -NoStart`
- **THEN** runner SHALL 校验服务可达并运行 Playwright 测试

#### Scenario: 缺少 API key
- **WHEN** 用户在未设置 OPENAI_API_KEY 的情况下运行 `.\scripts\run-tests.ps1`
- **THEN** runner SHALL 在启动测试前报告错误并退出

### Requirement: TESTCLAW-PW-WORKERS-010 Playwright E2E 并行 worker

Playwright E2E 测试 SHALL 使用按 CPU 核数 50% 配置的并行 worker，最少 1 个，最多 4 个。`fullyParallel` 选项 SHALL 被启用。

Vitest 后端测试 SHALL 保持顺序执行（`fileParallelism: false`、`sequence.sequential: true`），因为它们直接访问不支持并发写入的内部 SQLite gateway store。

#### Scenario: Playwright 以多个 worker 运行
- **WHEN** 用户运行 Playwright E2E 测试
- **THEN** worker 数 SHALL 设置为 `Math.min(4, Math.max(1, Math.floor(os.cpus().length * 0.5)))`
- **AND** `fullyParallel` SHALL 为 `true`

#### Scenario: Vitest 保持顺序执行
- **WHEN** 用户运行 Vitest 后端测试
- **THEN** `fileParallelism` SHALL 为 `false`
- **AND** `sequence.sequential` SHALL 为 `true`
