...
**触发机制**：aging cycle SHALL 由配置驱动的后台 schedule、受控管理触发或测试触发启动。schedule 默认 MUST 为 disabled；启用后的默认周期 MUST 表达为每日低峰时段一次，具体配置由 `nextAgent.memory.aging.*` 命名空间提供。`enabled=false` 时所有 trigger MUST 返回 skipped diagnostic；`enabled=true` 且 `schedule` 未设置时，本地后台 scheduler MUST NOT 启动，但受控管理触发和测试触发 MAY 执行；`enabled=true` 且 `schedule` 设置时，本地后台 scheduler MAY 启动。

**配置契约**：
- `nextAgent.memory.aging.enabled`：默认 `false`。
- `nextAgent.memory.aging.schedule`：cron 表达式，默认未设置（关闭）；示例 `0 0 3 * * ?` 每日凌晨 3 点。
...
**归档与实施前置条件**：
- `add-ts-memory-core` 的 memory core boundary MUST 先在当前代码基线可用并通过验证。core MUST 提供 `transitionLongTermMemoryState`、`adjustLongTermMemoryConfidence`、`markLongTermMemoryAccessed` 等 lifecycle mutation 方法，aging 直接调用。core MUST also expose `listLongTermMemory` lifecycle filters (`stateFilter`, `isPinned`, `maxLastAccessedAt`, `maxArchivedAt`) and owner-authorized `getLongTermMemoryDetail` access to retained ARCHIVED records. 归档顺序按 OpenSpec release 流程处理，不替代源码/测试核验。
- `add-ts-memory-configuration` MUST 先注册并校验 `nextAgent.memory.aging.*` 配置字段，aging 只消费已验证冻结快照。
- `add-ts-memory-tools` 的 `get_memory_detail` owner-authorized L2 detail access boundary MUST 先存在，revival-on-access 才能接线。后续 maintenance detail API 若存在，MUST 复用同一个 revival helper，不得成为当前 change 的并行前置路径。
...
- `ARCHIVED`：默认不参与普通检索；仅在 core time-range retrieval 语义允许时可见；可被 `getLongTermMemoryDetail` 访问触发复活（通过 `transitionLongTermMemoryState(targetState=ACTIVE)`）。保留期满后进入 deletion。
...
1. 通过 `store.listLongTermMemory({ stateFilter: "ACTIVE", isPinned: false, maxLastAccessedAt })` 扫描 `lastAccessedAt <= now - decayStaleDays` 的 records。对每条记录：`newConfidence = max(0, oldConfidence - decayFactor)`。若 `newConfidence = 0`，通过 `transitionLongTermMemoryState(targetState=ARCHIVED, archiveReason="confidence_decayed")` 归档；否则通过 `adjustLongTermMemoryConfidence` 写回新 confidence。
2. 通过 `store.listLongTermMemory({ stateFilter: "ARCHIVED", isPinned: false, maxArchivedAt })` 扫描 `archivedAt <= now - archiveRetentionDays` 的 records。通过 `deleteLongTermMemory` 物理删除 scoped record 和检索索引。
3. `getLongTermMemoryDetail` 访问 `ARCHIVED` 记录时，core detail 读取只返回 retained record 并更新 access telemetry；aging helper 再通过 `transitionLongTermMemoryState(targetState=ACTIVE)` 复活，并通过 `adjustLongTermMemoryConfidence(delta=+0.1)` 提升 confidence（上限 1.0）。revival 不在 schedule cycle 中批量执行。
4. 所有状态更新保持 `sourceTrace`、`createdAt` 和 scope 不变。
...
- **THEN** 系统 MUST 通过 `adjustLongTermMemoryConfidence(delta=-0.05)` 将 confidence 更新为 `0.55`
...
- **AND** `decayFactor=0.05`
- **THEN** 系统 MUST 通过 `transitionLongTermMemoryState(targetState=ARCHIVED, archiveReason="confidence_decayed")` 归档该 record
