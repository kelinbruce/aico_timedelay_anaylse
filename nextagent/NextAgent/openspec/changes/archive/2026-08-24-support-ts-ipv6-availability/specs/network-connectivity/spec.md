...
- **变更类型**：新增
- **目标内容**：系统校验并应用进程监听配置，提供可连接的 IPv6/双栈入站地址与启动提示，并保证首批关键出站集成路径能够访问 IPv6 literal 目标。
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`、`IPv6 入站监听提供可连接的启动地址`、`网络监听不得放宽已选择的安全暴露边界`、`首批关键出站路径可连接 IPv6 literal 目标`

### 前置条件

- **变更类型**：新增
- **目标内容**：部署环境启用目标 IP 协议栈；双栈 wildcard 场景还要求主机网络栈允许 IPv4-mapped IPv6；启动配置和所选产品入口的安全边界有效；出站目标已提供有效且获准访问的 URL。
- **依据 Requirements**：`IPv6 入站监听提供可连接的启动地址`、`网络监听不得放宽已选择的安全暴露边界`、`首批关键出站路径可连接 IPv6 literal 目标`

### 输入

- **变更类型**：新增
- **目标内容**：YAML 监听配置、可选进程监听环境变量，以及首批关键出站路径接收的 HTTP/HTTPS URL。
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`、`首批关键出站路径可连接 IPv6 literal 目标`

### 输出

- **变更类型**：新增
- **目标内容**：冻结的有效监听结果、可连接的启动地址、入站服务结果，以及首批关键出站路径的既有成功或安全失败结果。
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`、`IPv6 入站监听提供可连接的启动地址`、`网络监听不得放宽已选择的安全暴露边界`、`首批关键出站路径可连接 IPv6 literal 目标`

### 处理过程

- **变更类型**：新增
- **目标内容**：系统先按确定优先级解析并校验监听输入，再校验所选入口的安全暴露边界；通过后发布可连接的启动地址并提供入站服务，出站调用按其 URL 访问 IPv6 目标。
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`、`IPv6 入站监听提供可连接的启动地址`、`网络监听不得放宽已选择的安全暴露边界`、`首批关键出站路径可连接 IPv6 literal 目标`

### 结果

- **变更类型**：新增
- **目标内容**：有效输入产生 IPv6/双栈网络可用结果，并由真实 socket 验证形成合规证据；非法监听输入、安全边界冲突或不可达目标产生对应的安全失败结果。
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`、`IPv6 入站监听提供可连接的启动地址`、`网络监听不得放宽已选择的安全暴露边界`、`首批关键出站路径可连接 IPv6 literal 目标`、`IPv6 可用性必须由真实 socket 验证`
...
- **变更类型**：新增
- **原规格值**：不适用（新增）
- **目标规格值**：`NEXTAGENT_CHANNEL_HOST`、`NEXTAGENT_CHANNEL_PORT`；分别覆盖对应 YAML 结果，非法值在 ready 前失败
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`

- **规格项**：默认监听地址
- **变更类型**：新增
- **原规格值**：不适用（新增）
- **目标规格值**：未提供监听环境变量时为 `127.0.0.1:3000`
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`

- **规格项**：入站 IPv6 范围
- **变更类型**：新增
- **原规格值**：不适用（新增）
- **目标规格值**：`::1` 提供 IPv6 loopback；`::` 在支持的 Windows/Linux release package 目标且主机允许 IPv4-mapped IPv6 时提供 IPv6 与 IPv4 loopback
- **依据 Requirements**：`IPv6 入站监听提供可连接的启动地址`

- **规格项**：首批 IPv6 出站范围
- **变更类型**：新增
- **原规格值**：不适用（新增）
- **目标规格值**：模型提供方调用、api-call、task callback 支持 IPv6 literal HTTP/HTTPS 目标
- **依据 Requirements**：`首批关键出站路径可连接 IPv6 literal 目标`

### 接口

- **变更类型**：新增
- **目标内容**：进程环境变量 `NEXTAGENT_CHANNEL_HOST`、`NEXTAGENT_CHANNEL_PORT`，以及首批关键出站路径既有的 HTTP/HTTPS URL 输入。
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`、`首批关键出站路径可连接 IPv6 literal 目标`
...
- **变更类型**：新增
- **目标内容**：`F-10.5 集成外部系统`
- **依据 Requirements**：`IPv6 入站监听提供可连接的启动地址`、`首批关键出站路径可连接 IPv6 literal 目标`

### 主规格

- **变更类型**：新增
- **目标内容**：`network-connectivity`
- **依据 Requirements**：`监听地址和端口支持进程环境变量覆盖`、`IPv6 入站监听提供可连接的启动地址`、`网络监听不得放宽已选择的安全暴露边界`、`首批关键出站路径可连接 IPv6 literal 目标`、`IPv6 可用性必须由真实 socket 验证`
