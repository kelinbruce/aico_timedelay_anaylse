# add-ts-invoked-agent-discovery
...
状态：ready
类型：实施 change
主要 owner：`agent-capability`、`agent-core`
依赖：`add-ts-capability-core-governance`、`add-ts-agent-package-assembly`

目标：
- 支持内置 Agent capability 和 `agents/{agentId}/subagents/` 两个来源进入统一 catalog；远端 AgentRegistry discovery 后置。
