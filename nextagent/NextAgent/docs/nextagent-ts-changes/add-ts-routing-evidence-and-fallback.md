# add-ts-routing-evidence-and-fallback
...
类型：实施 change
主要 owner：`agent-core`
依赖：`add-ts-agent-routing-core`、`add-ts-routing-constraint-validation`、`add-ts-targeted-skill-routing`

目标：
- 支持确定性路径、模型驱动路径、拒绝、澄清或人工接管的选择证据和回退原因，并将 routing evidence 写入审计与脱敏日志。
