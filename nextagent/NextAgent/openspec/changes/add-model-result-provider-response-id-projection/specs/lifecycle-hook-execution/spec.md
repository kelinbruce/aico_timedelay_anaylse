...
成功 `ModelFinalResult` 携带 `usage` 时，`AFTER_MODEL_RESULT` boundary MUST 投影同一 `usage` 对象中已提供的 `inputTokens`、`outputTokens` 和 `totalTokens`；每个已提供字段 MUST 保持原始非负安全整数值，未提供字段 MUST 保持缺失。成功结果未携带 `usage` 时，boundary MUST 省略 `usage`。系统 MUST NOT 估算、补齐或从其他 token 字段推导 usage。

成功 `ModelFinalResult` 携带 `providerResponseId` 时，`AFTER_MODEL_RESULT` boundary MUST 投影同一 `providerResponseId` 值。成功结果未携带 `providerResponseId` 时，boundary MUST 省略该字段。系统 MUST NOT 合成、估算或从其他字段推导 `providerResponseId`。`providerResponseId` 只用于安全 response correlation，MUST NOT 进入 Web API、SSE、WebSocket、timeline、SafeError、audit、metric、trace 或 `ObservabilityObservationEvent`。

`modelE2ELatencyMs`、`firstContentLatencyMs`、`usage` 和 `providerResponseId` 仅是 observe-only boundary facts，MUST NOT 成为 mutation fields；hook 对这些字段返回的任何修改 MUST NOT 改变模型结果。
...
#### Scenario: 投影 provider 返回的 response ID

- **WHEN** 成功 `ModelFinalResult` 携带 `providerResponseId`
- **THEN** `AFTER_MODEL_RESULT` boundary MUST 投影同一 `providerResponseId` 值
- **AND** 该字段 MUST NOT 成为 mutation field
- **AND** hook 对该字段返回的任何修改 MUST NOT 改变模型结果

#### Scenario: Provider 未返回 response ID

- **WHEN** 成功 `ModelFinalResult` 未携带 `providerResponseId`
- **THEN** `AFTER_MODEL_RESULT` boundary MUST 省略 `providerResponseId`
- **AND** 系统 MUST NOT 合成或推导 response ID

#### Scenario: 模型调用失败

- **WHEN** concrete provider invocation 未返回成功 terminal result
- **THEN** 系统 MUST NOT 合成 `AFTER_MODEL_RESULT` boundary
- **AND** MUST NOT 以失败时刻合成 model E2E、first feedback、usage 或 `providerResponseId` 诊断事实

## Function 变更汇总

### 规格

- **规格项**：`AFTER_MODEL_RESULT` 诊断事实
- **变更类型**：修改
- **原规格值**：成功调用包含 `modelE2ELatencyMs`；出现可识别反馈时包含 `firstContentLatencyMs`；provider 提供 usage 时原样投影已提供字段，未提供保持缺失；诊断字段为 observe-only，不参与 mutation
- **目标规格值**：成功调用包含 `modelE2ELatencyMs`；出现可识别反馈时包含 `firstContentLatencyMs`；provider 提供 usage 时原样投影已提供字段，未提供保持缺失；成功结果携带 `providerResponseId` 时原样投影该值，未携带时省略；诊断字段为 observe-only，不参与 mutation
- **依据 Requirements**：Stage-specific boundaries and mutations are minimal runtime contracts

### 主规格

- **变更类型**：修改
- **目标内容**：`lifecycle-hook-execution`
- **依据 Requirements**：Stage-specific boundaries and mutations are minimal runtime contracts
