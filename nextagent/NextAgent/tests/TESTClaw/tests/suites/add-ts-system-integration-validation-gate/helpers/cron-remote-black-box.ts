...
export async function runRemoteCronCallbackCase(): Promise<void> {
  ...
  await withRunScope({ outputBase: process.env.TESTCLAW_SYSTEM_INTEGRATION_OUTPUT_ROOT }, async (scope) => {
    ...
    const source = await readFile(new URL('../e2e/backend/fixtures/TC-SI-049-consumer.mjs', import.meta.url), 'utf8');
    ...
    if (result.code !== 0) {
      throw new Error(`cron-consumer-failed:${result.stderr.trim() || result.stdout.trim() || 'no-diagnostic'}`);
    }
    ...
    expect(observations).toMatchObject({
      signedCallbackAccepted: true,
      concurrentDeliveryCoalesced: true,
      replayCoalesced: true,
    ...
    await writePassingCaseEvidence({
      ...
      observations,
      canaries: [
        { category: 'credential', value: 'cron-callback-secret' },
