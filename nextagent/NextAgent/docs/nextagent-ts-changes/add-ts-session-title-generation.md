# add-ts-session-title-generation
...
状态：active
类型：实施 change
主要 owner：`agent-session`、`agent-context-engine`、`agent-channel-web`
依赖：`ship-ts-minimal-agent-kernel`、`add-ts-local-session-store`

目标：
- 在首个用户请求完成后用确定性规则提取问题短语生成 4-40 字符 session title，持久化并支持历史列表展示；生成不得调用模型，不得覆盖用户手动标题。
